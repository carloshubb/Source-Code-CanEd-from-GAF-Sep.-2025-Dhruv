<div style="padding:20px">
    ...
    <h3 style="margin: 10px 0; text-align: left; font-size: 24px; line-height: 24px; color: #000">
        Hello {{ htmlentities($emailData['first_name']) }} {{ htmlentities($emailData['last_name']) }},
    </h3>
    ...
    <p style="text-align: center;margin: 16px 0;">
        <a href="{{ url('/en/email-verify/' . $emailData['token'] . '/' . $emailData['email']) }}" 
            style="border-radius: 4px; background-color: #b1040e; padding: 12px 24px; font-size: 16px; font-weight: bold; color: #ffffff; text-decoration: none; display: inline-block;">
            Verify our email
        </a>
    </p>
    ...
    <p style="margin: 0 0 10px; text-align: left; font-size: 16px; color: #000;">
        ...
        <br>
        <a href="{{ url('/en/email-verify/' . $emailData['token'] . '/' . $emailData['email']) }}" target="_blank" style="color: #b1040e;">
            {{ env('APP_URL') }}/en/email-verify/{{ $emailData['token'] }}/{{ $emailData['email'] }}
        </a>
    </p>

    <p style="margin: 0 0 10px; text-align: left; font-size: 16px; color: #000;">
        Once you have verified your email, you will have full access to your account.
    </p>

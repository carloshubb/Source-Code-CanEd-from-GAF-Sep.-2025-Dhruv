...
<p style="margin: 0 0 10px; text-align: left; font-size: 16px; color: #000;">New school register<p>
<ul style="text-align: left;color: #000;">
    <li><strong>School Name:</strong> {{ isset($emailData['schoolDetail'][0]->school_name) ? $emailData['schoolDetail'][0]->school_name : ''}}</li>
    <li><strong>School Email:</strong> {{$emailData['email']}}</li>
</ul>
<hr class="cb-hr" />
<a href="{{ url('/'.getDefaultLanguage(1)['abbreviation'].'/admin/schools')}}" style="border-radius: 4px; background-color: #b1040e; padding: 12px 24px; font-size: 16px; font-weight: bold; color: #ffffff; text-decoration: none; display: inline-block;">Click here to activate school</a>

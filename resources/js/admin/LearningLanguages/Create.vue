            ...
            <header class="">
                <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between">
                        <h1 class="can-edu-h1">
                            {{ isFormEdit ? 'Edit' : 'Create' }} learning language
                        </h1>
                        <router-link :to="{ name: 'admin.learning_languages.index' }" class="can-edu-btn-fill">
                            Back
                        </router-link>
                    </div>
                </div>
                ...
                <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between">
                        <p class="block text-base lg:text-lg  font-FuturaMdCnBT text-primary">
                            Select language(s) of the learning language
                        </p>
                    </div>
                </div>
            </header>
...
export default {
    ...
    computed: {
        ...mapState({
            isFormEdit: (state) => state.learning_languages.isFormEdit,
    ...
    methods: {
        handleInput(value, key) {
            this.$store.commit("learning_languages/setState", {
                key,
                value
            });
            if (value) {
        ...
        handleMultipleInput(key, value, language) {
            this.$store.commit("learning_languages/updateState", {
                value: value,
                id: language.id,
                key,
            });
        ...
        addUpdateForm() {
            this.$store
                .dispatch("learning_languages/addUpdateForm")
      ...
      if (inputElement) {
        ...
        fetchLearningLanguages() {
            if (this.$route.params.id) {
                ...
                this.$store.commit("learning_languages/setIsFormEdit", 1);
                this.$store
                    .dispatch("learning_languages/fetchLearningLanguages", {
                        id: id,
                        url: `${process.env.MIX_ADMIN_API_URL}learning_languages/${id}?withLearningLanguageDetail=1`,
                    })
                    .then((res) => {
                        let keys = ['status'];
                        this.$store.commit("learning_languages/setState", {
                        ...
                        let data =
                            res.data.data && res.data.data.learning_language_detail ?
                                res.data.data.learning_language_detail : [];
                        let obj = {};
                        data.map((res) => {
    ...
    created() {
        this.$store.commit("learning_languages/resetForm");
        this.$store

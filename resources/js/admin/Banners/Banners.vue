      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div
          ...
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Banners</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link :to="{ name: 'admin.banners.create' }" class="can-edu-btn-fill">
              Add new banner
            </router-link>
          </div>
        </div>
        <div
          ...
          <div class="relative">
            ...
            <form autocomplete="off" @submit.prevent>
              <input type="text" id="table-search-banners"
                ...
                placeholder="Search for banners" v-model="quickSearch" />
            </form>
          </div>
        </div>
        <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="banner in banners" :key="banner.id">
                <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6 flex items-center space-x-2">
                  <img v-if="banner.banner_image" class="w-10 h-10 rounded-full" :src="'/' + banner.banner_image.path"
                    alt="Banner image" />
                ...
                <td class="px-3 py-4 text-sm text-gray-500 lg:table-cell">
                  <div class="flex items-center space-x-2">
                    <div :id="'shortcodeText' + banner.id" @click="selectText('shortcodeText' + banner.id)"
                      class="bg-blue-100 text-blue-800 text-xs font-semibold mr-2 px-2.5 py-0.5 rounded">
                      {{ banner.short_code }}
                    </div>
                    <button @click="copyToClipboard(banner.short_code, banner.id)"
                  ...
                  <span v-if="showCopied[banner.id]" class="text-xs font-medium text-green-600">Copied!</span>
                </td>

                <td
                  class="relative py-3.5 pl-3 pr-4 text-center text-sm font-medium sm:pr-6 space-x-2 whitespace-nowrap">
                  <div class="flex items-center gap-2">
                    <router-link :to="{ name: 'admin.banners.edit', params: { id: banner?.id } }" type="button"
                    ...
                    <a :to="{ name: 'admin.banners.edit', params: { id: banner.id } }" type="button"
                      data-modal-target="editUserModal" data-modal-show="editUserModal"
                      class="text-red-500 hover:text-red-700 cursor-pointer" data-tooltip="Delete language"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchBanners(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchBannersByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchBanners(pagination.next_page_url)">
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.banners.limit,
      form: (state) => state.banners.form,
      banners: (state) => state.banners.banners,
      pagination: (state) => state.banners.pagination,
      validationErros: (state) => state.banners.validationErros,
      searchParam: (state) => state.banners.searchParam,
  ...
  methods: {
    fetchBanners(page_url) {
      ...
      this.$store.dispatch("banners/fetchBanners", { url: page_url });
    },
    fetchBannersByPage(page) {
      const baseUrl = this.pagination.base_url || this.pagination.first_page_url.split("?")[0];
      const queryParams = `?page=${page}`;
      const fullUrl = `${baseUrl}${queryParams}`;
      this.fetchEvents(fullUrl);
    },
    ...
    updateLimit(value) {
      this.$store.commit("banners/setLimit", value);
    ...
    deletebanner(banner) {
      this.$swal
        .fire({
          ...
          preConfirm: (password) => {
            return this.$store
              .dispatch("banners/deleteBanners", {
                id: banner.id,
                password: password,
              })
              .then((res) => {
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("banners/setSearchParam", this.quickSearch);
      this.$store.dispatch("banners/fetchBanners");
    }, 500),
    ...
    copyToClipboard(text, id) {
      if (navigator.clipboard && window.isSecureContext) {
        ...
        navigator.clipboard
          .writeText(text)
          .then(() => {
            this.showCopied[id] = true;
            setTimeout(() => (this.showCopied[id] = false), 2000);
          })
          .catch((err) => {
  ...
  created() {
    this.$store.commit("banners/setSearchParam", "");
    this.$store.commit("banners/setLimit", 100);
    this.$store.commit("banners/setSortBy", "banner_type");
    this.$store.commit("banners/setSortType", "asc");

    const page = this.$route.query.page || 1;
    const baseUrl = process.env.MIX_ADMIN_API_URL + "banners";
    this.fetchBanners(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("banners/fetchBanners");
  },
  watch: {

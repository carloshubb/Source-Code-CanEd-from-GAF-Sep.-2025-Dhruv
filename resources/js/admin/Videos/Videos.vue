      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div
          ...
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Videos</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link :to="{ name: 'admin.videos.create' }" class="can-edu-btn-fill">
              Add video
            </router-link>
          </div>
        </div>
        <div
          ...
          <div class="relative">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-videos-none">
            ...
            <form autocomplete="off" @submit.prevent>
              <input type="text" id="table-search-videos"
                ...
                placeholder="Search for videos" v-model="quickSearch" />
            </form>
          </div>
        </div>
        <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="video in videos" :key="video.id">
                <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6">
                  <img v-if="video.image" class="w-10 h-10 rounded-full" :src="'/' + video.image.path"
                    alt="Article Category image" />

                  <div class="font-medium text-gray-900 text-base">
                    {{
                      video.video_detail && video.video_detail[0]
                        ? video.video_detail[0].title
                        : ""
                    }}
                  </div>
                </td>
                ...
                <td
                  class="relative py-3.5 pl-3 pr-4 text-center text-sm font-medium whitespace-nowrap sm:pr-6 space-x-2">
                  <div class="flex items-center gap-2">
                    ...
                    <a :to="{
                      ...
                      class="text-red-500 hover:text-red-700 cursor-pointer" @click.prevent="deletevideo(video)">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchVideos(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchVideosByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchVideos(pagination.next_page_url)">
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.videos.limit,
      ...
      searchParam: (state) => state.videos.searchParam,
  ...
  methods: {
    fetchVideos(page_url) {
      ...
      this.$store.dispatch("videos/fetchVideos", { url: page_url });
    },
    fetchVideosByPage(page) {
      const baseUrl = this.pagination.base_url || this.pagination.first_page_url.split("?")[0];
      const queryParams = `?page=${page}`;
      const fullUrl = `${baseUrl}${queryParams}`;
      this.fetchVideos(fullUrl);
    },
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("videos/setSearchParam", this.quickSearch);
      this.$store.dispatch("videos/fetchVideos");
    }, 500),
  },
  created() {
    this.$store.commit("videos/setSearchParam", "");
    this.$store.commit("videos/setLimit", 100);
    this.$store.commit("videos/setSortBy", "video_title");
    this.$store.commit("videos/setSortType", "asc");
    const page = this.$route.query.page || 1;
    const baseUrl = process.env.MIX_ADMIN_API_URL + "videos";
    this.fetchVideos(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("videos/fetchVideos");
  },
  watch: {

      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div
          ...
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">School types</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link :to="{ name: 'admin.school_types.create' }" class="can-edu-btn-fill">
              Add new school types
            </router-link>
          </div>
        </div>
        <div
          ...
          <div class="relative">
            ...
            <form autocomplete="off" @submit.prevent>
              <input type="text" id="table-search-school_types"
                ...
                placeholder="Search for School types" v-model="quickSearch" />
            </form>
          </div>
        </div>
        <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="school_type in school_types"
                :key="school_type.id">
                <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6">
                  <div class="font-medium text-gray-900 text-base">
                    {{
                      school_type.school_type_detail &&
                        school_type.school_type_detail[0]
                ...
                <td
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <a :to="{
                      ...
                      @click.prevent="deleteschool_type(school_type)">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchSchoolTypes(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchSchoolTypesByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchSchoolTypes(pagination.next_page_url)">
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.school_types.limit,
      form: (state) => state.school_types.form,
      school_types: (state) => state.school_types.school_types,
      pagination: (state) => state.school_types.pagination,
      validationErros: (state) => state.school_types.validationErros,
      searchParam: (state) => state.school_types.searchParam,
      loading: (state) => state.school_types.loading,
    }),
  },
  data() {
    return {
  ...
  methods: {
    fetchSchoolTypes(page_url) {
      if (!page_url) return;
      const url = new URL(page_url);
      const page = url.searchParams.get("page") || 1;

      this.$router.push({ query: { ...this.$route.query, page } });
      this.$store.dispatch("school_types/fetchSchoolTypes", { url: page_url });
    },
    fetchSchoolTypesByPage(page) {
      const baseUrl = this.pagination.base_url || this.pagination.first_page_url.split("?")[0];
      const queryParams = `?page=${page}`;
      const fullUrl = `${baseUrl}${queryParams}`;
      this.fetchSchoolTypes(fullUrl);
    },
    // getPageNumbers() {
    //   const pages = [];
    ...
    updateLimit(value) {
      this.$store.commit("school_types/setLimit", value);
      this.$store.dispatch("school_types/fetchSchoolTypes");
    },
    deleteschool_type(record) {
      this.$swal
        .fire({
          ...
          preConfirm: (password) => {
            return this.$store
              .dispatch("school_types/deleteSchoolTypes", {
                id: record.id,
                password: password,
              })
              .then((res) => {
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("school_types/setSearchParam", this.quickSearch);
      this.$store.dispatch("school_types/fetchSchoolTypes");
    }, 500),
  },
  created() {
    this.$store.commit("school_types/setSearchParam", "");
    this.$store.commit("school_types/setLimit", 100);
    this.$store.commit("school_types/setSortBy", "school_type_name");
    this.$store.commit("school_types/setSortType", "asc");
    const page = this.$route.query.page || 1;
    const baseUrl = process.env.MIX_ADMIN_API_URL + "school_types";
    this.fetchSchoolTypes(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("school_types/fetchSchoolTypes");
  },
  watch: {

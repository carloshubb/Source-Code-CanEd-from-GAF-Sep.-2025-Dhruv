      ...
      <div class="px-4 sm:px-6 lg:px-8">
        ...
        >
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">School scholarships</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link
              :to="{ name: 'admin.school_scholarships.create' }"
              class="can-edu-btn-fill"
            >
              Add school scholarship
            </router-link>
          </div>
        </div>
        <div
        ...
        >
          ...
          <div class="relative">
            <div
              class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-school_scholarships-none"
            >
              <svg
            ...
            <form autocomplete="off" @submit.prevent>
              <input
              type="text"
              id="table-search-school_scholarships"
              ...
              placeholder="Search for school scholarships"
              v-model="quickSearch"
            />
            </form>
          </div>
          ...
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr
                ...
                v-for="school_scholarship in school_scholarships"
                :key="school_scholarship.id"
              >
                <td
                  class="relative py-4 pl-4 pr-3 text-sm sm:pl-6 flex items-center space-x-2"
                >
                  <!-- <img
                                        v-if="school_scholarship.image"
                                        class="w-10 h-10 rounded-full"
                                        :src="school_scholarship.image"
                                        alt="scholarship image"
                                    /> -->

                  <div class="font-medium text-gray-900 text-base">
                    {{
                      school_scholarship.school_scholarship_detail?.length > 0
                        ? school_scholarship.school_scholarship_detail[0].name
                        : ""
                    }}
                  </div>
                </td>
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="font-medium text-gray-900 text-base">
                    {{ school_scholarship ? school_scholarship.province : "" }}
                  </div>
                </td>
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="font-medium text-gray-900 text-base">
                    {{ school_scholarship ? school_scholarship.amount : "" }}
                  </div>
                </td>
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="font-medium text-gray-900 text-base">
                    {{
                      school_scholarship ? school_scholarship.expiry_date : ""
                    }}
                  </div>
                </td>
                ...
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="font-medium text-gray-900 text-base">
                    {{
                      school_scholarship ? school_scholarship.deadline_date : ""
                    }}
                  </div>
                </td>
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="font-medium text-gray-900 text-base">
                    {{
                      school_scholarship ? school_scholarship.date_posted : ""
                    }}
                  </div>
                </td>
                ...
                >
                  ...
                  <a
                    ...
                    @click="deleteschool_scholarship(school_scholarship)"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchSchoolScholarships(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchSchoolScholarshipsByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchSchoolScholarships(pagination.next_page_url)">
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.school_scholarships.limit,
      ...
      validationErros: (state) => state.school_scholarships.validationErros,
  ...
  methods: {
    fetchSchoolScholarships(page_url) {
      if (!page_url) return;
      const url = new URL(page_url);
      const page = url.searchParams.get("page") || 1;

      this.$router.push({ query: { ...this.$route.query, page } });
      this.$store.dispatch("school_scholarships/fetchSchoolScholarships", {
        url: page_url,
      });
    },
    fetchSchoolScholarshipsByPage(page) {
      const baseUrl = this.pagination.base_url || this.pagination.first_page_url.split("?")[0];
      const queryParams = `?page=${page}`;
      const fullUrl = `${baseUrl}${queryParams}`;
      this.fetchSchoolScholarships(fullUrl);
    },
    ...
    updateLimit(value) {
      this.$store.commit("school_scholarships/setLimit", value);
      this.$store.dispatch("school_scholarships/fetchSchoolScholarships");
    },
    deleteschool_scholarship(record) {
      this.$swal
        .fire({
          ...
          preConfirm: (password) => {
            return this.$store
              .dispatch("school_scholarships/deleteSchool", {
                id: record.id,
                password: password,
              })
              .then((res) => {
    ...
    deactiveSchool(school_scholarship, type) {
      this.$swal
        .fire({
          title: "Are you sure?",
        ...
        .then((result) => {
          if (result.isConfirmed) {
            this.$store
              .dispatch("school_scholarships/deactiveSchool", {
                id: school_scholarship.id,
                type: type,
              })
              .then((res) => {
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit(
        "school_scholarships/setSearchParam",
        this.quickSearch
      );
      this.$store.dispatch("school_scholarships/fetchSchoolScholarships");
    }, 500),
  ...
  created() {
    this.$store.commit("school_scholarships/setSearchParam", "");
    this.$store.commit("school_scholarships/setLimit", 100);
    this.$store.commit(
      "school_scholarships/setSortBy",
      "school_scholarship_name"
    );
    this.$store.commit("school_scholarships/setSortType", "asc");
    const page = this.$route.query.page || 1;
    const baseUrl = process.env.MIX_ADMIN_API_URL + "school_scholarships";
    this.fetchSchoolScholarships(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("school_scholarships/fetchSchoolScholarships");
  },
  watch: {

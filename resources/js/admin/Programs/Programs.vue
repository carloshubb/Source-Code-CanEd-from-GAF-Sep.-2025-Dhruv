      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div
          ...
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Programs</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link :to="{ name: 'admin.programs.create' }" class="can-edu-btn-fill">
              Add program
            </router-link>
          </div>
        </div>
        <div
          ...
          <div class="relative">
            ...
            <form autocomplete="off" @submit.prevent>
              <input type="text" id="table-search-programs"
                ...
                placeholder="Search for programs" v-model="quickSearch" />
            </form>
          </div>
        </div>
        <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="program in programs"
                :key="program.id">
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="font-normal text-gray-900 text-base">
                    {{
                      program.program_detail && program.program_detail[0]
                ...
                <td
                  ...
                  <a :to="{
                    ...
                    class="text-red-500 hover:text-red-700 cursor-pointer" @click.prevent="deleteprogram(program)">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                      stroke="currentColor" class="w-5 h-5">
                      <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchPrograms(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchProgramsByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchPrograms(pagination.next_page_url)">
...
import ProgramSetting from "../Pages/ProgramSetting.vue";
export default {
  components: {
    LoadingTable,
    ProgramSetting,
  ...
  computed: {
    ...mapState({
      limit: (state) => state.programs.limit,
      form: (state) => state.programs.form,
      programs: (state) => state.programs.programs,
      pagination: (state) => state.programs.pagination,
      validationErros: (state) => state.programs.validationErros,
      searchParam: (state) => state.programs.searchParam,
      loading: (state) => state.programs.loading,
    }),
  },
  data() {
    return {
  ...
  methods: {
    ...
    updateLimit(value) {
      this.$store.commit("programs/setLimit", value);
    ...
    deleteprogram(program) {
      this.$swal
        .fire({
          ...
          preConfirm: (password) => {
            return this.$store
              .dispatch("programs/deleteProgram", {
                id: program.id,
                password: password,
              })
              .then((res) => {
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("programs/setSearchParam", this.quickSearch);
      this.$store.dispatch("programs/fetchPrograms");
    }, 500),
  },
  created() {
    this.$store.commit("programs/setSearchParam", "");
    this.$store.commit("programs/setLimit", 100);
    this.$store.commit("programs/setSortBy", "program_name");
    this.$store.commit("programs/setSortType", "asc");

    const page = this.$route.query.page || 1;
        const baseUrl = process.env.MIX_ADMIN_API_URL + "programs";
        this.fetchPrograms(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("programs/fetchPrograms");
  },
  watch: {

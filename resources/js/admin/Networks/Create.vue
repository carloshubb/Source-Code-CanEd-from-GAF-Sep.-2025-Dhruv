            ...
            <header class="">
                <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between">
                        <h1 class="can-edu-h1">{{ isFormEdit ? 'Edit' : 'Create' }} network</h1>
                        <router-link :to="{ name: 'admin.networks.index' }" class="can-edu-btn-fill">
                            Back
                        </router-link>
                    </div>
                </div>
                ...
                <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between">
                        <p class="block text-base lg:text-lg  font-FuturaMdCnBT text-primary">
                            Select language(s) of the network
                        </p>
                    </div>
                </div>
            </header>
            <form class="px-4 md:px-6 lg:px-8" @submit.prevent="addUpdateForm()">
                ...
                <div class="grid my-5 md:grid-cols-2 md:gap-6" v-for="language in languages" :key="language.id" :class="(activeTab == null && language.is_default) ||
                    ...
                    <div class="relative z-0 w-full group md:col-span-2">
                        <label for="full_name" class="block text-base lg:text-lg">Full Name<span class="text-primary">*</span></label>
                        <input type="text" name="full_name" id="full_name"
                            ...
                            placeholder=" " @input="
                                ...
                                " :value="form['full_name'] &&
                                    form['full_name'][`full_name_${language.id}`]
                                    ? form['full_name'][`full_name_${language.id}`]
                                    : ''
                                    " />
                    ...
                    <div class="relative z-0 w-full group">
                        <label for="state_province" class="block text-base lg:text-lg">Your location<span class="text-primary">*</span></label>
                        <input type="text" name="state_province" id="state_province"
                            ...
                            @input="handleInput($event.target.value, 'state_province')"
                            :value="form['state_province'] ? form['state_province'] : ''" />
                        <p class="mt-2 text-base text-primary" v-if="validationErros.has(`state_province`)"
                            v-text="validationErros.get(`state_province`)"></p>
                    </div>
                    <div class="relative z-0 w-full group">
                        <label for="image" class="block text-base lg:text-lg">Photo / Image<span class="text-primary">*</span></label>
...
export default {
    computed: {
        ...mapState({
            isFormEdit: (state) => state.networks.isFormEdit,
    ...
    methods: {
        handleInput(value, key) {
            this.$store.commit("networks/setState", {
                key,
                value,
            });
            if (value.trim()) {
        ...
        handleSelectionChange(language, key) {
            ...
            this.$store.commit(`networks/updateState`, {
                value: content,
                id: language.id,
                key: key,
            });
        ...
        addUpdateForm() {
            this.$store.dispatch("networks/addUpdateForm").then(() =>
                this.$router.push({
        ...
        fetchNetworks() {
            if (this.$route.params.id) {
                ...
                this.$store.commit("networks/setIsFormEdit", 1);
                this.$store
                    .dispatch("networks/fetchNetworks", {
                        id: id,
                        url: `${process.env.MIX_ADMIN_API_URL}networks/${id}?withNetworkDetail=1`,
                    })
                    .then((res) => {
                        let keys = [
                            "image",
                        ...
                        let data =
                            res.data.data && res.data.data.network_detail
                                ? res.data.data.network_detail
                                : [];

                        let obj = {};
                        data.map((res) => {
                            obj["full_name_" + res.language_id] = res.full_name;
                        });
                        this.$store.commit("networks/setState", {
                            key: "full_name",
                            value: obj,
        ...
        handleFilePondFlagIconProcess(error, file) {
            this.$store.commit("networks/setForm", {
                image: file.serverId,
    ...
    created() {
        ...
        this.$store.commit("networks/resetForm");
        this.$store

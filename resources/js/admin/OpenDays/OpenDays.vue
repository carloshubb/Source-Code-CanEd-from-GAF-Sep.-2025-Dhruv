      ...
      <div class="px-4 sm:px-6 lg:px-8">
        ...
        >
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Open house days</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <!-- <router-link
                            :to="{ name: 'admin.open_days.create' }"
        ...
        >
          ...
          <div class="relative">
            <div
              class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-open_days-none"
            >
              <svg
        ...
        <form autocomplete="off" @submit.prevent>
          ...
          placeholder="Search for Open house days"
          v-model="quickSearch"
           autocomplete="off"
        />
        </form>
          ...
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              ...
              <!-- <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                ...
                <td
                  class="relative py-4 pl-4 pr-3 text-sm sm:pl-6 align-middle"
                >
                  <div class="flex items-center space-x-2 w-max">
                    <img
                      v-if="open_days.image"
                      class="w-10 h-10 rounded-full"
                      :src="open_days.image"
                      alt="scholarship image"
                    />
                    <div class="font-medium text-gray-900 text-base">
                      {{ open_days.school?.school_detail[0]?.school_name }}
                    </div>
                  </div>
                </td>
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="font-medium text-gray-900 text-base">
                    {{ open_days.OpenDayDetail[0]?.title }}

                  </div>
                </td>
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div
                    class="font-medium text-gray-900 text-base whitespace-nowrap"
                  >
                    {{ open_days.date }}
                  </div>
                </td>
                ...
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="font-medium text-gray-900 text-base">
                    {{ open_days.status == "pending" ? "in active" : "active" }}
                  </div>
                </td>
                ...
                >
                  ...
                  <a
                    ...
                    @click="deleteopen_days(open_days)"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchOpenDays(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchOpenDaysByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchOpenDays(pagination.next_page_url)">
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.open_days.limit,
  ...
  methods: {
    fetchOpenDays(page_url) {
      if (!page_url) return;
      const url = new URL(page_url);
      const page = url.searchParams.get("page") || 1;

      this.$router.push({ query: { ...this.$route.query, page } });
      this.$store.dispatch("open_days/fetchOpenDays", {
        url: page_url,
      });
    },
    fetchOpenDaysByPage(page) {
      const baseUrl = this.pagination.base_url || this.pagination.first_page_url.split("?")[0];
      const queryParams = `?page=${page}`;
      const fullUrl = `${baseUrl}${queryParams}`;
      this.fetchOpenDays(fullUrl);
    },
    getPageNumbers() {
      const pages = [];
    ...
    updateLimit(value) {
      this.$store.commit("open_days/setLimit", value);
      this.$store.dispatch("open_days/fetchOpenDays");
    },
    deleteopen_days(open_days) {
      this.$swal
        .fire({
          ...
          preConfirm: (password) => {
            return this.$store
              .dispatch("open_days/deleteVirtualTour", {
                id: open_days.id,
                password: password,
              })
              .then((res) => {
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("open_days/setSearchParam", this.quickSearch);
      this.$store.dispatch("open_days/fetchOpenDays");
    }, 500),
  },
  created() {
    this.$store.commit("open_days/setSearchParam", "");
    this.$store.commit("open_days/setLimit", 100);
    this.$store.commit("open_days/setSortBy", "city");
    this.$store.commit("open_days/setSortType", "asc");
    const page = this.$route.query.page || 1;
    const baseUrl = process.env.MIX_ADMIN_API_URL + "open_days";
    this.fetchOpenDays(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("open_days/fetchOpenDays");
  },
  watch: {

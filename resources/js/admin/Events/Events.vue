      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div
          ...
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Events</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link :to="{ name: 'admin.events.create' }" class="can-edu-btn-fill">
              Add event
            </router-link>
          </div>
        </div>
          ...
          <div class="relative">
            ...
            <form autocomplete="off" @submit.prevent>
              <input type="text" id="table-search-events"
                ...
                placeholder="Search for events" v-model="quickSearch" />
            </form>
          </div>
        </div>
        <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="event in events" :key="event.id">
                <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6">
                  <img v-if="event.image" class="w-10 h-10 rounded-full" :src="'/' + event.image.path"
                    alt="Article Category image" />

                  <div class="font-normal text-gray-900 text-base">
                    {{ event.event_detail && event.event_detail[0] ? event.event_detail[0].title : '' }}
                  </div>
                </td>
                ...
                <td
                  ...
                  <div class="flex items-center gap-2">
                    <router-link :to="{ name: 'admin.events.edit', params: { id: event?.id } }" type="button"
                    ...
                    <a :to="{ name: 'admin.events.edit', params: { id: event.id } }" type="button"
                      data-modal-target="editUserModal" data-modal-show="editUserModal"
                      class="text-red-500 hover:text-red-700 cursor-pointer" @click.prevent="deleteevent(event)">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchEvents(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchEventsByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchEvents(pagination.next_page_url)">
...
export default {
  ...
  methods: {
    fetchEvents(page_url) {
      ...
      this.$store.dispatch("events/fetchEvents", { url: page_url });
    },
  ...
  created() {
    this.$store.commit("events/setSearchParam", "");
    this.$store.commit("events/setLimit", 100);
    this.$store.commit("events/setSortBy", "event_name");
    this.$store.commit("events/setSortType", "asc");

    const page = this.$route.query.page || 1;
    const baseUrl = process.env.MIX_ADMIN_API_URL + "events";
    this.fetchEvents(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("events/fetchEvents");
  },
  watch: {

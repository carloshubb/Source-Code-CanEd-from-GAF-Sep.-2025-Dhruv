    ...
    <AppLayout>
        <div class="relative shadow-md rounded-lg bg-white py-4">
            ...
            <div class="flex flex-col sm:flex-col md:flex-row lg:flex-row items-center justify-between gap-2 bg-white dark:bg-gray-800">
                ...
                <h1 class="can-edu-h1">Articles</h1>
                </div>
                <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                    <router-link :to="{ name: 'admin.articles.create'}" class="can-edu-btn-fill">
                            Add new article
                        </router-link>
                </div>
            </div>
            <div class="flex flex-col sm:flex-col md:flex-row lg:flex-row items-center justify-between py-4 gap-2 bg-white dark:bg-gray-800">
                ...
                <div class="relative">
                    ...
                    <form autocomplete="off" @submit.prevent>
                        <input type="text" id="table-search-articles" class="block p-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg w-auto md:w-64 lg:w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search for articles" v-model="quickSearch" />
                    </form>
                </div>
            </div>
            <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
                ...
                <tbody class="divide-y divide-gray-200 bg-white">
                    <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="article in articles" :key="article.id">
                        <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6 flex items-center space-x-2">
                            <img v-if="article.article_image" class="w-10 h-10 rounded-full" :src="'/'+article.article_image.path" alt="Article Category image" />
                             <div class="font-normal text-gray-900 text-base word_break w-40">
                                {{article.article_detail && article.article_detail[0] ? article.article_detail[0].name : ''}}
                            </div>
                        </td>
                        <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6">
                            <div class="font-normal text-gray-900 text-base line-clamp-3" v-html="article?.article_detail?.[0]?.short_description">
                            </div>
                        </td>
                        <td class="relative py-3.5 pl-3 pr-4 text-center text-sm font-medium sm:pr-6 space-x-2 whitespace-nowrap whitespace-nowrap">
                            <div class="flex items-center gap-2">
                            <router-link :to="{ name: 'admin.articles.edit', params: {id: article?.id}}" type="button" data-modal-target="editUserModal" data-modal-show="editUserModal" class="text-gray-500 hover:text-gray-700 cursor-pointer">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                </svg>
                            </router-link>
                            <a :to="{ name: 'admin.articles.edit', params: {id: article.id}}" type="button" data-modal-target="editUserModal" data-modal-show="editUserModal" class="text-red-500 hover:text-red-700 cursor-pointer" @click.prevent="deletearticle(article)">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                        ...
                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                            ...
                            <template v-for="page in getPageNumbers()" :key="page">
                                ...
                                <a v-else href="#" 
                                   ...
                                   @click.prevent="fetchArticlesByPage(page)">
                                    {{ page }}
                                </a>
                            </template>
                            ...
                            <a href="#" 
                               ...
                               @click.prevent="fetchArticles(pagination.next_page_url)">
...
export default {
    ...
    computed: {
        ...mapState({
            ...
            searchParam: (state) => state.articles.searchParam,
...
//   this.$router.push({ query: { ...this.$route.query, page } }); // Add this line
//   this.$store.dispatch("articles/fetchArticles", { url: page_url });
//         },
fetchArticles(page_url) {
        this.$store.dispatch("articles/fetchArticles", { url: page_url })
            .then(() => {
    ...
    fetchArticlesByPage(page) {
        const baseUrl = this.pagination.base_url || 
                       this.pagination.first_page_url.split('?')[0];
        const fullUrl = `${baseUrl}?page=${page}`;
        this.fetchArticles(fullUrl);
    },
    
    updateUrlWithCurrentPage() {
        if (!this.pagination?.current_page) return;
        
        const currentPage = this.pagination.current_page;
        const newUrl = new URL(window.location.href);
        
        // Always set page number (even if it's 1)
        newUrl.searchParams.set('page', currentPage);
        
        // Remove page param if it's 1 (optional - choose one approach)
        // if (currentPage === 1) {
    ...
    getPageNumbers() {
      ...
      return pages;
        ...
        updateLimit(value) {
            this.$store.commit("articles/setLimit", value);
      ...
      this.$swal
        ...
        quickSearchFilter: _.debounce(function () {
            this.$store.commit("articles/setSearchParam", this.quickSearch);
            this.$store.dispatch("articles/fetchArticles");
        }, 500),
    },
    created() {
        this.$store.commit("articles/setSearchParam", "");
        this.$store.commit("articles/setLimit", 100);
        this.$store.commit("articles/setSortBy", "article_name");
        this.$store.commit("articles/setSortType", "asc");
//         const page = this.$route.query.page || 1;
//   const baseUrl = process.env.MIX_ADMIN_API_URL + "articles";
//   this.fetchArticles(`${baseUrl}?page=${page}`);
const urlParams = new URLSearchParams(window.location.search);
    const pageParam = urlParams.get('page');
    
    let initialUrl = null;
    if (pageParam) {
        const baseUrl = '/api/articles'; // Adjust to your API endpoint
        initialUrl = `${baseUrl}?page=${pageParam}`;
    }
    
    this.$store.dispatch("articles/fetchArticles", { url: initialUrl });
        // this.$store.dispatch("articles/fetchArticles");
    },
    watch: {

      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div
          ...
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Quotes</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link :to="{ name: 'admin.quotes.create' }" class="can-edu-btn-fill">
              Add quotes
            </router-link>
          </div>
        </div>
        <div
          ...
          <div class="relative">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-quotes-none">
            ...
            <form autocomplete="off" @submit.prevent>
              <input type="text" id="table-search-quotes"
                ...
                placeholder="Search for quotes" v-model="quickSearch" />
            </form>
          </div>
        </div>
        <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="quotes in quotes" :key="quotes.id">
                <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6 flex items-center space-x-2">
                  <img v-if="quotes.quote_image" class="w-10 h-10 rounded-full" :src="'/' + quotes.quote_image.path"
                    alt="quote image" />

                  <div class="font-normal text-gray-900 text-base" v-html="quotes?.quote_detail?.[0]?.quote"></div>
                </td>
                ...
                <td
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <a :to="{
                      ...
                      class="text-red-500 hover:text-red-700 cursor-pointer" @click.prquotes="deletequotes(quotes)">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchQuotes(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchQuotesByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchQuotes(pagination.next_page_url)">
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.quotes.limit,
      form: (state) => state.quotes.form,
      quotes: (state) => state.quotes.quotes,
      pagination: (state) => state.quotes.pagination,
      validationErros: (state) => state.quotes.validationErros,
      searchParam: (state) => state.quotes.searchParam,
      loading: (state) => state.quotes.loading,
    }),
  },
  data() {
    return {
  ...
  methods: {
    ...
    updateLimit(value) {
      this.$store.commit("quotes/setLimit", value);
      this.$store.dispatch("quotes/fetchQuotes");
    },
    deletequotes(quote) {
      this.$swal
        .fire({
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("quotes/setSearchParam", this.quickSearch);
      this.$store.dispatch("quotes/fetchQuotes");
    }, 500),
  },
  created() {
    this.$store.commit("quotes/setSearchParam", "");
    this.$store.commit("quotes/setLimit", 100);
    this.$store.commit("quotes/setSortBy", "quote");
    this.$store.commit("quotes/setSortType", "asc");
    const page = this.$route.query.page || 1;
    const baseUrl = process.env.MIX_ADMIN_API_URL + "quotes";
    this.fetchQuotes(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("quotes/fetchQuotes");
  },
  watch: {

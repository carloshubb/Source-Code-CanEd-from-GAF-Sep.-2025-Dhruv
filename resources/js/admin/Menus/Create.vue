            ...
            <header class="">
                <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between">
                        ...
                        <router-link :to="{ name: 'admin.menus.index' }" class="can-edu-btn-fill">
                            Back
                        </router-link>
                    </div>
                </div>
            ...
            <form class="px-4 md:px-6 lg:px-8" @submit.prevent="addUpdateForm()">
                <div class="grid my-5 grid-cols-1 sm:grid-cols-1 md:grid-cols-2  lg:grid-cols-2 gap-6 ">
                    <div class="relative z-0 w-full group">
                        <label class="block text-base lg:text-lg" for="name">Name<span class="text-primary">*</span></label>
                    ...
                    <div class="relative z-0 w-full group">
                        <label>&nbsp;</label>
                        <fieldset>
                            <legend class="sr-only">Is top menu?</legend>
                            <div class="flex items-center mb-4">
                                <input id="is_top_menu" name="is_top_menu" type="checkbox" value=""
                                    class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2"
                                    :checked="is_top_menu" v-model="is_top_menu" />
                                <label for="is_top_menu" class="ml-2 text-sm font-medium text-gray-900">Set as top
                                    menu</label>
                            </div>
                        </fieldset>
                        <p class="mt-2 text-sm text-red-400" v-if="validationErros.has('is_top_menu')"
                            v-text="validationErros.get('is_top_menu')"></p>

                    </div>
                </div>
                <ListErrors :validationErrors="validationErros" />
                <button type="submit" class="can-edu-btn-fill mb-2" :disabled="loading">Submit</button>
            </form>
        </div>
...
export default {
    computed: {
        ...
        is_top_menu: {
            get: function () {
                return this.$store.state.menus.form.is_top_menu;
            },
            set: function (val) {
                this.$store.commit('menus/setForm', {
                    is_top_menu: val

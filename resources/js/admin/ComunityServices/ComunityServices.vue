      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div
          ...
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Community services</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link :to="{ name: 'admin.comunity_services.create' }" class="can-edu-btn-fill">
        ...
        <div
          ...
          <div class="relative">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-comunity_services-none">
            ...
            <form autocomplete="off" @submit.prevent>
              <input type="text" id="table-search-comunity_services"
                ...
                placeholder="Search for community services" v-model="quickSearch" />
            </form>
          </div>
        </div>
          ...
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50"
                v-for="comunity_service in comunity_services" :key="comunity_service.id">
                <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6">
                  <div class="font-normal text-gray-900 text-base">
                    {{ comunity_service.comunity_service_detail && comunity_service.comunity_service_detail[0] ?
                      comunity_service.comunity_service_detail[0].name : ''}}
                  </div>
                </td>

                <td
                  class="relative py-3.5 pl-3 pr-4 text-center text-sm font-medium sm:pr-6 space-x-2 whitespace-nowrap"
                  v-if="comunity_service?.id != 1">
                  <div class="flex items-center gap-2">
                    <router-link :to="{ name: 'admin.comunity_services.edit', params: { id: comunity_service?.id } }"
                    ...
                    <a :to="{ name: 'admin.comunity_services.edit', params: { id: comunity_service.id } }" type="button"
                      data-modal-target="editUserModal" data-modal-show="editUserModal"
                      class="text-red-500 hover:text-red-700 cursor-pointer"
                      @click.prcomunity_service="deletecomunity_service(comunity_service)">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchComunityServices(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchComunityServicesByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchComunityServices(pagination.next_page_url)">
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.comunity_services.limit,
      form: (state) => state.comunity_services.form,
      comunity_services: (state) => state.comunity_services.comunity_services,
      pagination: (state) => state.comunity_services.pagination,
      validationErros: (state) => state.comunity_services.validationErros,
      searchParam: (state) => state.comunity_services.searchParam,
      loading: (state) => state.comunity_services.loading,
    }),
  },
  data() {
    return {
  ...
  methods: {
    fetchComunityServices(page_url) {
      if (!page_url) return;
      const url = new URL(page_url);
      const page = url.searchParams.get("page") || 1;

      this.$router.push({ query: { ...this.$route.query, page } });
      this.$store.dispatch("comunity_services/fetchComunityServices", { url: page_url });
    },
    fetchComunityServicesByPage(page) {
      const baseUrl = this.pagination.base_url || this.pagination.first_page_url.split("?")[0];
      const queryParams = `?page=${page}`;
      const fullUrl = `${baseUrl}${queryParams}`;
      this.fetchComunityServices(fullUrl);
    },
    ...
    updateLimit(value) {
      this.$store.commit("comunity_services/setLimit", value);
      this.$store.dispatch("comunity_services/fetchComunityServices");
    },
    deletecomunity_service(comunity_service) {
      this.$swal
        .fire({
          ...
          preConfirm: (password) => {
            return this.$store
              .dispatch("comunity_services/deleteComunityService", {
                id: comunity_service.id,
                password: password,
              })
              .then((res) => {
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("comunity_services/setSearchParam", this.quickSearch);
      this.$store.dispatch("comunity_services/fetchComunityServices");
    }, 500),
  },
  created() {
    this.$store.commit("comunity_services/setSearchParam", "");
    this.$store.commit("comunity_services/setLimit", 100);
    this.$store.commit("comunity_services/setSortBy", "comunity_service_name");
    this.$store.commit("comunity_services/setSortType", "asc");
    const page = this.$route.query.page || 1;
  const baseUrl = process.env.MIX_ADMIN_API_URL + "comunity_services";
  this.fetchComunityServices(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("comunity_services/fetchComunityServices");
  },
  watch: {

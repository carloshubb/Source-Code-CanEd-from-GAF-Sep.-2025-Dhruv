            ...
            <header class="">
                <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between">
                        <h1 class="can-edu-h1">
                            {{ isFormEdit ? "Edit" : "Create a new" }} community service
                        </h1>
                        <router-link :to="{ name: 'admin.comunity_services.index' }" class="can-edu-btn-fill">
                            Back
                        </router-link>
                    </div>
                </div>
                ...
                <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between">
                        <p class="block text-base lg:text-lg  font-FuturaMdCnBT text-primary">
                            Select language(s) of the community service
                        </p>
                    </div>
                </div>
            </header>
...
export default {
    ...
    computed: {
        ...mapState({
            isFormEdit: (state) => state.comunity_services.isFormEdit,
    ...
    methods: {
        handleInput(value, key) {
            this.$store.commit("comunity_services/setState", {
                key,
                value
            });
        ...
        handleMultipleInput(key, value, language) {
            this.$store.commit("comunity_services/updateState", {
                value: value,
                id: language.id,
                key,
            });
        ...
        addUpdateForm() {
            this.$store
                .dispatch("comunity_services/addUpdateForm")
        ...
        fetchComunityServices() {
            if (this.$route.params.id) {
                let id = this.$route.params.id;

                this.$store.commit("comunity_services/setIsFormEdit", 1);
                this.$store
                    .dispatch("comunity_services/fetchComunityServices", {
                        id: id,
                        url: `${process.env.MIX_ADMIN_API_URL}comunity_services/${id}?withComunityServiceDetail=1`,
                    })
                    .then((res) => {
                        let keys = ['status'];
                        this.$store.commit("comunity_services/setState", {
                        ...
                        let data =
                            res.data.data && res.data.data.comunity_service_detail ?
                                res.data.data.comunity_service_detail : [];
                        let obj = {};
                        data.map((res) => {
    ...
    created() {
        this.$store.commit("comunity_services/resetForm");
        this.$store

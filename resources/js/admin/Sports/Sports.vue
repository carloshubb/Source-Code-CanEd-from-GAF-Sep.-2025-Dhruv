      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div
          ...
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Sports</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link :to="{ name: 'admin.sports.create' }" class="can-edu-btn-fill">
              Add sport
            </router-link>
          </div>
        </div>
        ...
        <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="sport in sports" :key="sport.id">
                <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6">
                  <div class="font-normal text-gray-900 text-base">
                    {{
                      sport.sport_detail && sport.sport_detail[0]
                ...
                <td
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <a :to="{
                      ...
                      class="text-red-500 hover:text-red-700 cursor-pointer" @click.prsport="deletesport(sport)">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchSports(pagination.prev_page_url)">
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchSports(pagination.next_page_url)">
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.sports.limit,
      form: (state) => state.sports.form,
      sports: (state) => state.sports.sports,
      pagination: (state) => state.sports.pagination,
      validationErros: (state) => state.sports.validationErros,
      searchParam: (state) => state.sports.searchParam,
      loading: (state) => state.sports.loading,
    }),
  },
  data() {
    return {
  ...
  methods: {
    ...
    updateLimit(value) {
      this.$store.commit("sports/setLimit", value);
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("sports/setSearchParam", this.quickSearch);
      this.$store.dispatch("sports/fetchSports");
    }, 500),
  },
  created() {
    this.$store.commit("sports/setSearchParam", "");
    this.$store.commit("sports/setLimit", 100);
    this.$store.commit("sports/setSortBy", "sport_name");
    this.$store.commit("sports/setSortType", "asc");

    const page = this.$route.query.page || 1;
        const baseUrl = process.env.MIX_ADMIN_API_URL + "sports";
        this.fetchSports(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("sports/fetchSports");
  },
  watch: {

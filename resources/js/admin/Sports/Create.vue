            ...
            <header class="">
                <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between">
                        <h1 class="can-edu-h1">
                            {{ isFormEdit ? 'Edit' : 'Create' }} sport
                        </h1>
                        <router-link :to="{ name: 'admin.sports.index' }" class="can-edu-btn-fill">
                            Back
                        </router-link>
                    </div>
                </div>
                ...
                <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between">
                        <p class="block text-base lg:text-lg  font-FuturaMdCnBT text-primary">
                            Select language(s) of the sport
                        </p>
                    </div>
                </div>
            </header>
...
export default {
    ...
    computed: {
        ...mapState({
            isFormEdit: (state) => state.sports.isFormEdit,
    ...
    methods: {
        handleInput(value, key) {
            this.$store.commit("sports/setState", {
                key,
                value
            });
            if (value.trim()) {
        ...
        handleMultipleInput(key, value, language) {
            this.$store.commit("sports/updateState", {
                value: value,
                id: language.id,
                key,
            });
        ...
        fetchSports() {
            if (this.$route.params.id) {
                ...
                this.$store.commit("sports/setIsFormEdit", 1);
                this.$store
                    .dispatch("sports/fetchSports", {
                        id: id,
                        url: `${process.env.MIX_ADMIN_API_URL}sports/${id}?withSportDetail=1`,
                    })
                    .then((res) => {
                        let keys = ['status'];
                        this.$store.commit("sports/setState", {
    ...
    created() {
        this.$store.commit("sports/resetForm");
        this.$store

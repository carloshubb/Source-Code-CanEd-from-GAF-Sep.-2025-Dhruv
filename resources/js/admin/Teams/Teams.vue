      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div
          ...
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Team</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link :to="{ name: 'admin.teams.create' }" class="can-edu-btn-fill">
              Add team member
            </router-link>
          </div>
        </div>
        <div
          ...
          <div class="relative">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-teams-none">
            ...
            <form autocomplete="off" @submit.prevent>
              <input type="text" id="table-search-teams"
                ...
                placeholder="Search for team" v-model="quickSearch" />
            </form>
          </div>
        </div>
        <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="team in teams" :key="team.id">
                <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6 flex items-center space-x-2">
                  <img v-if="team.image" class="w-10 h-10 rounded-full" :src="'/' + team.image.path"
                    alt="Article Category image" />

                  <div class="font-normal text-gray-900 text-base">
                    {{
                      team.team_detail && team.team_detail[0]
                        ? team.team_detail[0].name
                        : ""
                    }}
                  </div>
                </td>
                ...
                <td
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <a :to="{
                      ...
                      class="text-red-500 hover:text-red-700 cursor-pointer" @click.prteam="deleteteam(team)">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchTeams(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchTeamsByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchTeams(pagination.next_page_url)">
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.teams.limit,
      form: (state) => state.teams.form,
      teams: (state) => state.teams.teams,
      pagination: (state) => state.teams.pagination,
      validationErros: (state) => state.teams.validationErros,
      searchParam: (state) => state.teams.searchParam,
  ...
  methods: {
    ...
    updateLimit(value) {
      this.$store.commit("teams/setLimit", value);
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("teams/setSearchParam", this.quickSearch);
      this.$store.dispatch("teams/fetchTeams");
    }, 500),
  },
  created() {
    this.$store.commit("teams/setSearchParam", "");
    this.$store.commit("teams/setLimit", 100);
    this.$store.commit("teams/setSortBy", "team_title");
    this.$store.commit("teams/setSortType", "asc");

    const page = this.$route.query.page || 1;
        const baseUrl = process.env.MIX_ADMIN_API_URL + "teams";
        this.fetchTeams(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("teams/fetchTeams");
  },
  watch: {

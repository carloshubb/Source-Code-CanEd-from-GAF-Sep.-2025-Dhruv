        ...
        <header class="py-4 bg-white">
            <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between">
                    <h1 class="can-edu-h1">
                        {{isFormEdit ? 'Edit' : 'Create'}} social service
                    </h1>
                    <router-link :to="{ name: 'admin.social_services.index' }"
        ...
        <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex items-center justify-between">
            <p class="block text-base lg:text-lg  font-FuturaMdCnBT text-primary">
              Select language(s) of the social service
            </p>
          </div>
        </div>
      </header>
        <form class="py-4 px-4 bg-white" @submit.prevent="addUpdateForm()">
                ...
                <div class="relative z-0 w-full mb-6 group">
                    <input type="text" name="name" id="name"
                        ...
                        :value="form['name'] &&
                            ...
                            ''" />
                    <label for="name"
                        class="peer-focus:font-medium absolute text-base text-gray-700 font-semibold dark:text-gray-400 duration-300 transform -translate-y-6 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-primary peer-focus:dark:text-blue-500 peer-placeholder-shown:translate-y-0 peer-focus:-translate-y-6">Name</label>
            ...
            <div class="grid my-5 md:grid-cols-2 md:gap-6">
                <div class="relative z-0 w-full mb-6 group">
                    ...
                    <label for="status"
                        class="peer-focus:font-medium absolute text-base text-gray-700 font-semibold dark:text-gray-400 duration-300 transform -translate-y-6 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-primary peer-focus:dark:text-blue-500 peer-placeholder-shown:translate-y-0 peer-focus:-translate-y-6">Status</label>
...
import ListErrors from '../components/ListErrors.vue';
    ...
    ListErrors
        ...
        computed: {
            ...mapState({
                isFormEdit: (state) => state.social_services.isFormEdit,
        ...
        methods: {
            handleInput(value, key) {
                this.$store.commit("social_services/setState", {
                    key,
                    value
                });
            },
            handleMultipleInput(key, value, language) {
                this.$store.commit("social_services/updateState", {
                    value: value,
                    id: language.id,
                    key,
                });
            ...
            addUpdateForm() {
                this.$store
                    .dispatch("social_services/addUpdateForm")
            ...
            fetchSocialServices() {
                if (this.$route.params.id) {
                    ...
                    this.$store.commit("social_services/setIsFormEdit", 1);
                    this.$store
                        .dispatch("social_services/fetchSocialServices", {
                            id: id,
                            url: `${process.env.MIX_ADMIN_API_URL}social_services/${id}?withSportDetail=1`,
                        })
                        .then((res) => {
                            let keys = ['status'];
                            this.$store.commit("social_services/setState", {
        ...
        created() {
            this.$store.commit("social_services/resetForm");
            this.$store

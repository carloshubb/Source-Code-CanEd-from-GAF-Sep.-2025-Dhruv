      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div
          ...
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Pages</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link :to="{ name: 'admin.pages.create' }" class="can-edu-btn-fill">
              Add new page
            </router-link>
          </div>
        </div>
        <div
          ...
          <div class="relative">
            ...
            <form autocomplete="off" @submit.prevent>
              <input type="text" id="table-search-pages"
                ...
                placeholder="Search for Pages" v-model="quickSearch" />
            </form>
          </div>
        </div>
        <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="page in pages" :key="page.id">
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <img v-if="page.page_image" class="w-10 h-10 rounded-full" :src="page.page_image.path"
                    alt="Page Category image" />
                  ...
                  <div class="font-normal text-gray-900 text-base">
                    <!-- <a
                      :href="`${
                        page.is_home_page
                          ? `${app_url}`
                          : `${app_url}/en/${page.slug}`
                      } `"
                      target="_blank"
                    >
                      {{
                        page.page_detail && page.page_detail[0]
                          ? page.page_detail[0].name
                          : ""
                      }}
                    </a> -->
                    <!-- <a
                ...
                >
                    ...
                    <!-- <a :href="generatePageUrl(page)" target="_blank">
                      {{
                        page.page_detail && page.page_detail[0]
                          ? page.page_detail[0].name
                          : ""
                    ...
                    <a :href="page.page_detail && page.page_detail[0] && page.page_detail[0].slug === 'register' 
                    ? '/en/register?type=student' 
            ...
            {{
              page.page_detail && page.page_detail[0]
                ...
                <td
                  class="relative py-3.5 pl-3 pr-4 text-right text-sm font-medium sm:pr-6 flex justify-center whitespace-nowrap space-x-2">
                  <a :href="`pages/${page.id}/edit`" class="text-gray-500 hover:text-gray-700 cursor-pointer">
                  ...
                  <a type="button" data-modal-target="editUserModal" data-modal-show="editUserModal"
                    class="text-red-500 hover:text-red-700 cursor-pointer" @click.prevent="deletepage(page)">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                      stroke="currentColor" class="w-5 h-5">
                      <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchPages(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchPagesByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchPages(pagination.next_page_url)">
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.pages.limit,
      form: (state) => state.pages.form,
      pages: (state) => state.pages.pages,
      pagination: (state) => state.pages.pagination,
      validationErros: (state) => state.pages.validationErros,
      searchParam: (state) => state.pages.searchParam,
  ...
  methods: {
    generatePageUrl(page) {
      if (page.slug === "home" || page.slug === "home-pages") {
        return `${this.app_url}/en`;
      }
      return `${this.app_url}/en/${page.slug}`;
    },
    fetchPages(page_url) {
      if (!page_url) return;
      ...
      this.$store.dispatch("pages/fetchPages", { url: page_url });
    },
    fetchPagesByPage(page) {
      const baseUrl = this.pagination.base_url || this.pagination.first_page_url.split("?")[0];
      const queryParams = `?page=${page}`;
      const fullUrl = `${baseUrl}${queryParams}`;
      this.fetchPages(fullUrl);
    },
    ...
    updateLimit(value) {
      this.$store.commit("pages/setLimit", value);
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("pages/setSearchParam", this.quickSearch);
      this.$store.dispatch("pages/fetchPages");
    }, 500),
  },
  created() {
    this.$store.commit("pages/setSearchParam", "");
    this.$store.commit("pages/setLimit", 100);
    this.$store.commit("pages/setSortBy", "page_name");
    this.$store.commit("pages/setSortType", "asc");

    const page = this.$route.query.page || 1;
        const baseUrl = process.env.MIX_ADMIN_API_URL + "pages";
        this.fetchPages(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("pages/fetchPages");
  },
  watch: {

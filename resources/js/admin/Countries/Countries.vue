      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div
          ...
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Countries Status</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link :to="{ name: 'admin.countries.create' }" class="can-edu-btn-fill">
              Add Contries Status
            </router-link>
          </div>
        </div>
        ...
        <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="country in countries"
                :key="country.id">
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="font-normal text-gray-900 text-base">
                    {{
                      country.country_detail && country.country_detail[0]
                        ? country.country_detail[0].name
                        : ""
                    }}
                  </div>
                </td>
                ...
                <td
                  ...
                  <a :to="{
                    ...
                    class="text-red-500 hover:text-red-700 cursor-pointer" @click.prevent="deleteCountry(country)">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                      stroke="currentColor" class="w-5 h-5">
                      <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchCountries(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchCountriessByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchCountries(pagination.next_page_url)">
...
export default {
  ...
  methods: {
    fetchCountries(page_url) {
      ...
      this.$store.dispatch("countries/fetchCountries", { url: page_url });
    },
    fetchCountriessByPage(page) {
      const baseUrl = this.pagination.base_url || this.pagination.first_page_url.split("?")[0];
      const queryParams = `?page=${page}`;
      const fullUrl = `${baseUrl}${queryParams}`;
      this.fetchCountries(fullUrl);
    },
    getPageNumbers() {
      const pages = [];
    ...
    updateLimit(value) {
      this.$store.commit("countries/setLimit", value);
      this.$store.dispatch("countries/fetchCountries");
    },
    deleteCountry(program) {
      this.$swal
        .fire({
          ...
          preConfirm: (password) => {
            return this.$store
              .dispatch("countries/deleteCountry", {
                id: program.id,
                password: password,
              })
              .then((res) => {
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("countries/setSearchParam", this.quickSearch);
      this.$store.dispatch("countries/fetchCountries");
    }, 500),
  },
  created() {
    this.$store.commit("countries/setSearchParam", "");
    this.$store.commit("countries/setLimit", 100);
    this.$store.commit("countries/setSortBy", "name");
    this.$store.commit("countries/setSortType", "asc");

    const page = this.$route.query.page || 1;
    const baseUrl = process.env.MIX_ADMIN_API_URL + "countries";
    this.fetchCountries(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("countries/fetchCountries");
  },
  watch: {

      ...
      <div class="px-4 sm:px-6 lg:px-8">
        ...
        >
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Sponsors</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link
              :to="{ name: 'admin.sponsors.create' }"
        ...
        >
          ...
          <div class="relative">
            <div
              class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-sponsors-none"
            >
              <svg
        ...
        >
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              ...
              >
                <td
                  class="relative py-4 pl-4 pr-3 text-base sm:pl-6 flex items-center space-x-2"
                >
                  <img
                    v-if="sponsor.image"
                  ...
                  <div class="font-normal text-gray-900 text-base">
                    {{
                      sponsor.sponsor_detail && sponsor.sponsor_detail[0]
                        ? sponsor.sponsor_detail[0].title
                        : ""
                    }}
                  </div>
                </td>
                ...
                >
                  <div class="flex items-center gap-2">
                    ...
                    <a
                      ...
                      @click.prsponsor="deletesponsor(sponsor)"
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        fill="none"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchSponsors(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchSponsorsByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchSponsors(pagination.next_page_url)">
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.sponsors.limit,
      form: (state) => state.sponsors.form,
      sponsors: (state) => state.sponsors.sponsors,
      pagination: (state) => state.sponsors.pagination,
      validationErros: (state) => state.sponsors.validationErros,
      searchParam: (state) => state.sponsors.searchParam,
      loading: (state) => state.sponsors.loading,
    }),
  },
  data() {
    return {
  ...
  methods: {
    fetchSponsors(page_url) {
      if (!page_url) return;
      const url = new URL(page_url);
      const page = url.searchParams.get("page") || 1;

      this.$router.push({ query: { ...this.$route.query, page } });
      this.$store.dispatch("sponsors/fetchSponsors", { url: page_url });
    },
    fetchSponsorsByPage(page) {
      const baseUrl = this.pagination.base_url || this.pagination.first_page_url.split("?")[0];
      const queryParams = `?page=${page}`;
      const fullUrl = `${baseUrl}${queryParams}`;
      this.fetchSponsors(fullUrl);
    },
    ...
    updateLimit(value) {
      this.$store.commit("sponsors/setLimit", value);
    ...
    deletesponsor(record) {
      this.$swal
        .fire({
          ...
          preConfirm: (password) => {
            return this.$store
              .dispatch("sponsors/deleteSponsor", {
                id: record.id,
                password: password,
              })
              .then((res) => {
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("sponsors/setSearchParam", this.quickSearch);
      this.$store.dispatch("sponsors/fetchSponsors");
    }, 500),
  },
  created() {
    this.$store.commit("sponsors/setSearchParam", "");
    this.$store.commit("sponsors/setLimit", 100);
    this.$store.commit("sponsors/setSortBy", "sponsor_title");
    this.$store.commit("sponsors/setSortType", "asc");

    const page = this.$route.query.page || 1;
        const baseUrl = process.env.MIX_ADMIN_API_URL + "sponsors";
        this.fetchSponsors(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("sponsors/fetchSponsors");
  },
  watch: {

      ...
      <div class="px-4 sm:px-6 lg:px-8">
        ...
        >
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Careers</h1>
          </div>
              ...
              >
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="font-normal text-gray-900 text-base">
                    {{
                      career.career_detail && career.career_detail[0]
                        ? career.career_detail[0].class_name
                        : ""
                    }}
                  </div>
                </td>
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div
                    class="font-normal text-gray-900 text-base line-clamp-3"
                    v-html="career?.career_detail?.[0]?.class_definition"
                  ></div>
                </td>
                <!-- <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="font-normal text-gray-900 text-base">
                ...
                >
                  ...
                  <a
                    ...
                    @click.prevent="deletecareer(career)"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchCareers(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchCareersByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchCareers(pagination.next_page_url)">
...
import CareerSetting from "../Pages/CareerSetting.vue";
export default {
  components: {
    LoadingTable,
    CareerSetting,
  ...
  computed: {
    ...mapState({
      limit: (state) => state.careers.limit,
      form: (state) => state.careers.form,
      careers: (state) => state.careers.careers,
      pagination: (state) => state.careers.pagination,
      validationErros: (state) => state.careers.validationErros,
      searchParam: (state) => state.careers.searchParam,
      loading: (state) => state.careers.loading,
    }),
  },
  data() {
    return {
  ...
  methods: {
    fetchCareers(page_url) {
      // this.$store.dispatch("careers/fetchCareers", { url: page_url });
      if (!page_url) return;
  ...
  this.$store.dispatch("careers/fetchCareers", { url: page_url });
    ...
    updateLimit(value) {
      this.$store.commit("careers/setLimit", value);
      this.$store.dispatch("careers/fetchCareers");
    },
    deletecareer(career) {
      this.$swal
        .fire({
          ...
          preConfirm: (password) => {
            return this.$store
              .dispatch("careers/deletecareers", {
                id: career.id,
                password: password,
              })
              .then((res) => {
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("careers/setSearchParam", this.quickSearch);
      this.$store.dispatch("careers/fetchCareers");
    }, 500),
  },
  created() {
    this.$store.commit("careers/setSearchParam", "");
    this.$store.commit("careers/setLimit", 100);
    this.$store.commit("careers/setSortBy", "career_detail_name");
    this.$store.commit("careers/setSortType", "asc");
    const page = this.$route.query.page || 1;
  const baseUrl = process.env.MIX_ADMIN_API_URL + "careers";
  this.fetchCareers(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("careers/fetchCareers");
  },
  watch: {

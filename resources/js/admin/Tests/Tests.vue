      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div
          ...
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Tests taken</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link :to="{ name: 'admin.tests.create' }" class="can-edu-btn-fill">
              Add new test taken
            </router-link>
          </div>
        </div>
        <div
          ...
          <div class="relative">
            ...
            <form autocomplete="off" @submit.prevent>
              <input type="text" id="table-search-tests"
        ...
        <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="test in tests" :key="test.id">
                <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6">
                  <div class="font-medium text-gray-900 text-base">
                    {{
                      test.test_detail && test.test_detail[0]
                        ? test.test_detail[0].name
                        : ""
                    }}
                  </div>
                </td>
                ...
                <td
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <a :to="{
                      ...
                      class="text-red-500 hover:text-red-700 cursor-pointer" @click.prevent="deletetest(test)">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="Tests(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchTestsByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="Tests(pagination.next_page_url)">
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.tests.limit,
      form: (state) => state.tests.form,
      tests: (state) => state.tests.tests,
      pagination: (state) => state.tests.pagination,
      validationErros: (state) => state.tests.validationErros,
      searchParam: (state) => state.tests.searchParam,
      loading: (state) => state.tests.loading,
    }),
  },
  data() {
    return {
  ...
  methods: {
    Tests(page_url) {
      ...
      this.$store.dispatch("tests/fetchTests", { url: page_url });
    },
    fetchTestsByPage(page) {
    ...
    updateLimit(value) {
      this.$store.commit("tests/setLimit", value);
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("tests/setSearchParam", this.quickSearch);
      this.$store.dispatch("tests/fetchTests");
    }, 500),
  },
  created() {
    this.$store.commit("tests/setSearchParam", "");
    this.$store.commit("tests/setLimit", 100);
    this.$store.commit("tests/setSortBy", "test_name");
    this.$store.commit("tests/setSortType", "asc");
    const page = this.$route.query.page || 1;
    const baseUrl = process.env.MIX_ADMIN_API_URL + "tests";
    this.Tests(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("tests/fetchTests");
  },
  watch: {

<template>
        ...
        <div class="px-4 sm:px-6 lg:px-8">
          <div
            ...
            <div class="sm:flex-auto">
              <h1 class="can-edu-h1">Curricular activities</h1>
            </div>
            <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
              <router-link :to="{ name: 'admin.curricular_activities.create' }" class="can-edu-btn-fill">
          ...
          <div
            ...
            <div class="relative">
              <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-sports-none">
              ...
              <form autocomplete="off" @submit.prevent>
                <input type="text" id="table-search-sports"
                  ...
                  placeholder="Search for sports" v-model="quickSearch" />
              </form>
            </div>
          </div>
          <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
            <table class="min-w-full divide-y divide-gray-300 overflow-auto">
              ...
              <tbody class="divide-y divide-gray-200 bg-white">
                <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="curricular in curricular_activities" :key="curricular.id">
                  <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6">
                    <div class="font-normal text-gray-900 text-base">
                      {{
                        curricular.curricular_activity_detail && curricular.curricular_activity_detail[0]
                          ? curricular.curricular_activity_detail[0].name
                          : ""
                      }}
                    </div>
                  </td>
                  ...
                  <td
                    ...
                    v-if="curricular?.id != 1">
                    <div class="flex items-center gap-2">
                      <router-link :to="{
                        name: 'admin.curricular_activities.edit',
                        params: { id: curricular?.id },
                      ...
                      <a :to="{
                        ...
                        class="text-red-500 hover:text-red-700 cursor-pointer" @click.prsport="deletecurricular_activity(curricular_activity)">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                          stroke="currentColor" class="w-5 h-5">
                          <path stroke-linecap="round" stroke-linejoin="round"
              ...
              <div>
                <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                  <a href="#"
                    ...
                    @click.prevent="fetchCurricularActivities(pagination.prev_page_url)">
                  ...
                  <template v-for="page in getPageNumbers()" :key="page">
                    ...
                    <a v-else href="#"
                      ...
                      }" @click.prevent="fetchSportsByPage(page)">
                      {{ page }}
                    </a>
                  </template>
                  ...
                  <a href="#"
                    ...
                    @click.prevent="fetchCurricularActivities(pagination.next_page_url)">
  ...
  export default {
    ...
    computed: {
      ...mapState({
        limit: (state) => state.curricular_activities.limit,
        form: (state) => state.curricular_activities.form,
        curricular_activities: (state) => state.curricular_activities.curricular_activities,
        pagination: (state) => state.curricular_activities.pagination,
        validationErros: (state) => state.curricular_activities.validationErros,
        searchParam: (state) => state.curricular_activities.searchParam,
        loading: (state) => state.curricular_activities.loading,
      }),
    },
    data() {
      return {
    ...
    methods: {
        fetchCurricularActivities(page_url) {
        this.$store.dispatch("curricular_activities/fetchCurricularActivities", { url: page_url });
      },
      fetchSportsByPage(page) {
        const baseUrl = this.pagination.base_url || this.pagination.first_page_url.split("?")[0];
        const queryParams = `?page=${page}`;
        const fullUrl = `${baseUrl}${queryParams}`;
        this.fetchCurricularActivities(fullUrl);
      },
      // getPageNumbers() {
      //   const pages = [];
      ...
      updateLimit(value) {
        this.$store.commit("curricular_activities/setLimit", value);
        this.$store.dispatch("curricular_activities/fetchCurricularActivities");
      },
      deletecurricular_activity(curricular_activity) {
        this.$swal
          .fire({
            ...
            preConfirm: (password) => {
              return this.$store
                .dispatch("curricular_activities/deleteCurricularActivity", {
                  id: curricular_activity.id,
                  password: password,
                })
                .then((res) => {
      ...
      quickSearchFilter: _.debounce(function () {
        this.$store.commit("curricular_activities/setSearchParam", this.quickSearch);
        this.$store.dispatch("curricular_activities/fetchCurricularActivities");
      }, 500),
    },
    created() {
      this.$store.commit("curricular_activities/setSearchParam", "");
      this.$store.commit("curricular_activities/setLimit", 100);
      this.$store.commit("curricular_activities/setSortBy", "activity_name");
      this.$store.commit("curricular_activities/setSortType", "asc");
      this.$store.dispatch("curricular_activities/fetchCurricularActivities");
    },
    watch: {

            ...
            <div class="px-4 sm:px-6 lg:px-8">
                <div
                    ...
                    <div class="sm:flex-auto">
                        <h1 class="can-edu-h1">Languages</h1>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                        <router-link :to="{ name: 'admin.languages.create' }" class="can-edu-btn-fill">
                            Add new language
                        </router-link>
                    </div>
                </div>
                ...
                <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
                    <table class="min-w-full divide-y divide-gray-300 overflow-auto">
                        <thead>
                            <tr class="divide-x divide-gray-200">
                                <th scope="col"
                                    ...
                                    Name</th>
                                <th scope="col"
                                    class="sticky top-0 z-10 bg-primary backdrop-blur backdrop-filter hidden px-3 py-3.5 text-left text-lg lg:text-xl font-FuturaMdCnBT font-medium text-white lg:table-cell">
                                    Abbreviation</th>
                                <th scope="col"
                                    class="sticky top-0 z-10 bg-primary backdrop-blur backdrop-filter hidden px-3 py-3.5 text-left text-lg lg:text-xl font-FuturaMdCnBT font-medium text-white lg:table-cell">
                                    Native name</th>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="language in languages"
                                :key="language.id">
                                <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6">
                                    <div class="font-medium text-gray-900 text-base flex items-center space-x-2">
                                        <img v-if="language.flag_icon" class="w-10 h-10 rounded-full"
                                            :src="'/' + language.flag_icon.path" alt="Flag icon" />
                                        <span class="ml-2 text-base">{{ language.name }}</span>
                                        <span
                                            class="ml-2 bg-red-100 text-primary text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300"
                                ...
                                <td class="hidden px-3 py-3.5 text-base lg:table-cell">{{ language.abbreviation }}</td>
                                <td class="hidden px-3 py-3.5 text-base lg:table-cell">{{ language.native_name }}</td>
                                <td
                                    class="relative py-3.5 pl-3 pr-4 text-center text-sm font-medium sm:pr-6 space-x-2 whitespace-nowrap">
                                    <div class="flex items-center gap-2">

                                        <router-link :to="{ name: 'admin.languages.edit', params: { id: language.id } }"
                                            type="button" data-modal-target="editUserModal"
                                            data-modal-show="editUserModal" data-tooltip="Edit language"
                                        ...
                                        <a :to="{ name: 'admin.languages.edit', params: { id: language.id } }"
                                            type="button" data-modal-target="editUserModal"
                                            data-modal-show="editUserModal" data-tooltip="Delete language"
                    ...
                    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        ...
                        <div>
                            <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px"
                                aria-label="Pagination" v-if="pagination.next_page_url || pagination.prev_page_url">
                                <a href="#"
...
export default {
    ...
    methods: {
        ...
        deleteLanguage(language) {
            if (language.is_default == '1' || language.name === 'English') {
                this.$swal.fire({
                    icon: "warning",
    ...
    created() {
        ...
        const baseUrl = process.env.MIX_ADMIN_API_URL + "languages";
        this.fetchLanguages(`${baseUrl}?page=${page}`);
        // this.$store.dispatch("languages/fetchLanguages");
    },
    watch: {

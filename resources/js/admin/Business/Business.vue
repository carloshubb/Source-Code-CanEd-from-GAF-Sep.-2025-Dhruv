            ...
            <div class="px-4 sm:px-6 lg:px-8">
                <div
                    ...
                    <div class="sm:flex-auto">
                        <h1 class="can-edu-h1">Businesses</h1>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                        <router-link :to="{ name: 'admin.business.create' }" class="can-edu-btn-fill">
                            Add business
                        </router-link>
                    </div>
                </div>
                    ...
                    <div class="relative">
                        ...
                        <form autocomplete="off" @submit.prevent>
                            <input type="text" id="table-search-schools"
                                ...
                                placeholder="Search for businesses" v-model="quickSearch" />
                        </form>
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-300 overflow-auto">
                        <thead>
                            <tr class="divide-x divide-gray-200">
                                <th scope="col"
                                    ...
                                    Name</th>
                                <!-- <th scope="col" class="bg-primary backdrop-blur backdrop-filter py-3.5 pl-4 pr-3 text-left text-lg lg:text-xl font-FuturaMdCnBT font-medium text-white sm:pl-6">Website</th> -->
                                <th scope="col"
                                    class="bg-primary backdrop-blur backdrop-filter py-3.5 pl-4 pr-3 text-left text-lg lg:text-xl font-FuturaMdCnBT font-medium text-white sm:pl-6">
                                    Email</th>
                                <th scope="col"
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50"
                                v-for="business in businesses" :key="business.id">
                                <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6 flex items-center w-max space-x-2">
                                    <img v-if="getFirstImage(business.image)" class="w-10 h-10 rounded-full"
                                        :src="getFirstImage(business.image)" alt="Business image" />

                                    <div class="font-medium text-gray-900 text-base">
                                        {{ business.business_detail && business.business_detail[0] ?
                                            business.business_detail[0].business_name : ''}}
                                    </div>
                                </td>
                            ...
                            <div class="font-medium text-gray-900 text-base">
                                ...
                                <td class="p-4">
                                    <SwitchGroup as="div" class="flex items-center">
                                        <Switch @click="
                                            deactiveBusiness(
                                                business,
                                                business.deactive_account == 0 ? 1 : 0
                                            )
                                            " :class="[
                                        business.deactive_account == 0
                                            ? 'bg-primary'
                                            : 'bg-gray-200',
                                ...
                                <td class="relative py-3.5 px-4 text-center space-x-2 whitespace-nowrap space-y-2">
                                    <div class="flex items-center gap-2">
                                        <router-link :to="{ name: 'admin.business.edit', params: { id: business?.id } }"
                                        ...
                                        <a type="button" data-modal-target="editUserModal"
                                            data-modal-show="editUserModal"
                                            class="text-red-500 hover:text-red-700 cursor-pointer"
                                            data-tooltip="Delete language" @click.prschool="deletebusiness(business)">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                ...
                                <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6">
                                    <button class="can-edu-btn-fill" @click="sendCredentialsEmail(business.id)">
                                        Send Credentials
                                    </button>
                                </td>
                            </tr>
                      ...
                      <div>
                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                          <a href="#"
                            ...
                            @click.prevent="fetchBusinesses(pagination.prev_page_url)">
                          ...
                          <template v-for="page in getPageNumbers()" :key="page">
                            ...
                            <a v-else href="#"
                              ...
                              }" @click.prevent="fetchBusinessesByPage(page)">
                              {{ page }}
                            </a>
                          </template>
                          ...
                          <a href="#"
                            ...
                            @click.prevent="fetchBusinesses(pagination.next_page_url)">
...
export default {
    ...
    methods: {
        async sendCredentialsEmail(businessId) {
        try {
            const apiUrl = process.env.MIX_ADMIN_API_URL + "send-business-credentials/" + businessId;
    ...
    closeModalOnOutsideClick(event) {
        ...
        fetchBusinesses(page_url) {
            if (!page_url) return;
      const url = new URL(page_url);
      const page = url.searchParams.get("page") || 1;
      this.$router.push({ query: { ...this.$route.query, page } });
            this.$store.dispatch("businesses/fetchBusinesses", { url: page_url });
        },
        fetchBusinessesByPage(page) {
      const baseUrl = this.pagination.base_url || this.pagination.first_page_url.split("?")[0];
      const queryParams = `?page=${page}`;
      const fullUrl = `${baseUrl}${queryParams}`;
      this.fetchBusinesses(fullUrl);
    },
    ...
    getPageNumbers() {
      ...
      return pages;
        ...
        updateLimit(value) {
            this.$store.commit("businesses/setLimit", value);
        ...
        deletebusiness(business) {
            this.$swal
                .fire({
                    ...
                    preConfirm: (password) => {
                        return this.$store
                            .dispatch("businesses/deleteBusiness", {
                                id: business.id,
                                password: password,
                            })
                            .then((res) => {
        ...
        deactiveBusiness(business, type) {
            this.$swal
                .fire({
                    title: "Are you sure?",
                ...
                .then((result) => {
                    if (result.isConfirmed) {
                        this.$store.dispatch("businesses/deactiveBusiness", {
                            id: business.id,
                            type: type,
                        })
                            .then((res) => {
        ...
        quickSearchFilter: _.debounce(function () {
            this.$store.commit("businesses/setSearchParam", this.quickSearch);
            this.$store.dispatch("businesses/fetchBusinesses");
        }, 500),
    ...
    created() {
        this.$store.commit("businesses/setSearchParam", "");
        this.$store.commit("businesses/setLimit", 100);
        this.$store.commit("businesses/setSortBy", "business_name");
        this.$store.commit("businesses/setSortType", "asc");
        const page = this.$route.query.page || 1;
    const baseUrl = process.env.MIX_ADMIN_API_URL + "businesses";
    this.fetchBusinesses(`${baseUrl}?page=${page}`);
        // this.$store.dispatch("businesses/fetchBusinesses");
    },
    watch: {

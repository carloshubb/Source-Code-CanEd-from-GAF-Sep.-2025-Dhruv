  ...
  <AppLayout>
    <div class="relative shadow-md rounded-lg bg-white py-4">
      <div class="px-4 sm:px-6 lg:px-8">
        <div
          class="flex flex-col sm:flex-col md:flex-row lg:flex-row items-center justify-between gap-2 dark:bg-gray-800">
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Messaging app</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link :to="{ name: 'admin.messaging_apps.create' }" class="can-edu-btn-fill">
              Add new messaging app
            </router-link>
          </div>
        </div>
        <div
          class="flex flex-col sm:flex-col md:flex-row lg:flex-row items-center justify-between py-4 gap-2 dark:bg-gray-800">
          <div>
            show
          ...
          <div class="relative">
            ...
            <form autocomplete="off" @submit.prevent>
              <input type="text" id="table-search-messaging_apps"
                ...
                placeholder="Search for messaging apps" v-model="quickSearch" />
            </form>
          </div>
        </div>
        <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="messaging_app in messaging_apps"
                :key="messaging_app.id">
                <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6">
                  <div class="font-medium text-gray-900 text-base">
                    {{
                      messaging_app.messaging_app_detail &&
                        messaging_app.messaging_app_detail[0]
                        ? messaging_app.messaging_app_detail[0].name
                        : ""
                    }}
                  </div>
                </td>
                ...
                <td
                  ...
                  <div class="flex items-center gap-2">
                    ...
                    <a :to="{
                      ...
                      @click.prevent="deletemessaging_app(messaging_app)">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="MessagingApps(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchMessaginAppsByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="MessagingApps(pagination.next_page_url)">
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.messaging_apps.limit,
      form: (state) => state.messaging_apps.form,
      messaging_apps: (state) => state.messaging_apps.messaging_apps,
      pagination: (state) => state.messaging_apps.pagination,
      validationErros: (state) => state.messaging_apps.validationErros,
      searchParam: (state) => state.messaging_apps.searchParam,
      loading: (state) => state.messaging_apps.loading,
    }),
  },
  data() {
    return {
  ...
  methods: {
    MessagingApps(page_url) {
      if (!page_url) return;
      const url = new URL(page_url);
      const page = url.searchParams.get("page") || 1;

      this.$router.push({ query: { ...this.$route.query, page } });
      this.$store.dispatch("messaging_apps/fetchMessagingApps", {
        url: page_url,
      });
    },
    fetchMessaginAppsByPage(page) {
      const baseUrl = this.pagination.base_url || this.pagination.first_page_url.split("?")[0];
      const queryParams = `?page=${page}`;
      const fullUrl = `${baseUrl}${queryParams}`;
      this.MessagingApps(fullUrl);
    },
    ...
    updateLimit(value) {
      this.$store.commit("messaging_apps/setLimit", value);
      this.$store.dispatch("messaging_apps/fetchMessagingApps");
    },
    deletemessaging_app(messaging_app) {
      this.$swal
        .fire({
          ...
          preConfirm: (password) => {
            return this.$store
              .dispatch("messaging_apps/deleteMessagingApp", {
                id: messaging_app.id,
                password: password,
              })
              .then((res) => {
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("messaging_apps/setSearchParam", this.quickSearch);
      this.$store.dispatch("messaging_apps/fetchMessagingApps");
    }, 500),
  },
  created() {
    this.$store.commit("messaging_apps/setSearchParam", "");
    this.$store.commit("messaging_apps/setLimit", 100);
    this.$store.commit("messaging_apps/setSortBy", "messaging_app_name");
    this.$store.commit("messaging_apps/setSortType", "asc");

    const page = this.$route.query.page || 1;
        const baseUrl = process.env.MIX_ADMIN_API_URL + "messaging_apps";
        this.MessagingApps(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("messaging_apps/fetchMessagingApps");
  },
  watch: {

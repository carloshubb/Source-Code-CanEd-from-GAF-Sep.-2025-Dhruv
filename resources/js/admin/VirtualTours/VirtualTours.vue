      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <div
          ...
          <div class="sm:flex-auto">
            <h1 class="can-edu-h1">Virtual tours</h1>
          </div>
          <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <router-link :to="{ name: 'admin.virtual_tours.create' }" class="can-edu-btn-fill">
              Add virtual tours
            </router-link>
          </div>
        </div>
        <div
          ...
          <div class="relative">
            <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-virtual_tours-none">
            ...
            <form autocomplete="off" @submit.prevent>
              <input type="text" id="table-search-virtual_tours"
                ...
                placeholder="Search for virtual tours" v-model="quickSearch" />
            </form>
          </div>
        </div>
        <div class="md:-mx-4 mt-4 ring-1 ring-gray-300 sm:mx-0 sm:rounded-lg overflow-auto">
          <table class="min-w-full divide-y divide-gray-300 overflow-auto">
            ...
            <tbody class="divide-y divide-gray-200 bg-white">
              <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50" v-for="virtual_tours in virtual_tours"
                :key="virtual_tours.id">
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <img v-if="virtual_tours.image" class="w-10 h-10 rounded-full" :src="virtual_tours.image"
                    alt="scholarship image" />
                </td>
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  {{ virtual_tours.school?.school_detail[0]?.school_name }}

                </td>
                <td class="relative py-4 pl-4 pr-3 text-sm sm:pl-6">
                  <div class="font-medium text-gray-900 text-base line-clamp-2">
                    {{
                      virtual_tours?.virtual_tour_detail?.length > 0
                        ? virtual_tours?.virtual_tour_detail[0].description
                        : ""
                    }}
                  </div>
                </td>
                ...
                <td
                  ...
                  <a type="button" data-modal-target="editUserModal" data-modal-show="editUserModal"
                    class="text-red-500 hover:text-red-700 cursor-pointer" @click="deletevirtual_tours(virtual_tours)">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                      stroke="currentColor" class="w-5 h-5">
                      <path stroke-linecap="round" stroke-linejoin="round"
            ...
            <div>
              <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                <a href="#"
                  ...
                  @click.prevent="fetchVirtualTours(pagination.prev_page_url)">
                ...
                <template v-for="page in getPageNumbers()" :key="page">
                  ...
                  <a v-else href="#"
                    ...
                    }" @click.prevent="fetchVirtualToursByPage(page)">
                    {{ page }}
                  </a>
                </template>
                ...
                <a href="#"
                  ...
                  @click.prevent="fetchVirtualTours(pagination.next_page_url)">
...
export default {
  ...
  computed: {
    ...mapState({
      limit: (state) => state.virtual_tours.limit,
  ...
  methods: {
    fetchVirtualTours(page_url) {
      if (!page_url) return;
      const url = new URL(page_url);
      const page = url.searchParams.get("page") || 1;
      this.$router.push({ query: { ...this.$route.query, page } });
      this.$store.dispatch("virtual_tours/fetchVirtualTours", {
        url: page_url,
      });
    },
    fetchVirtualToursByPage(page) {
      const baseUrl = this.pagination.base_url || this.pagination.first_page_url.split("?")[0];
      const queryParams = `?page=${page}`;
      const fullUrl = `${baseUrl}${queryParams}`;
      this.fetchVirtualTours(fullUrl);
    },
    // getPageNumbers() {
    //   const pages = [];
    ...
    updateLimit(value) {
      this.$store.commit("virtual_tours/setLimit", value);
      this.$store.dispatch("virtual_tours/fetchVirtualTours");
    },
    deletevirtual_tours(record) {
      this.$swal
        .fire({
    ...
    quickSearchFilter: _.debounce(function () {
      this.$store.commit("virtual_tours/setSearchParam", this.quickSearch);
      this.$store.dispatch("virtual_tours/fetchVirtualTours");
    }, 500),
  },
  created() {
    this.$store.commit("virtual_tours/setSearchParam", "");
    this.$store.commit("virtual_tours/setLimit", 100);
    this.$store.commit("virtual_tours/setSortBy", "virtual_tour_desc");
    this.$store.commit("virtual_tours/setSortType", "asc");

    const page = this.$route.query.page || 1;
    const baseUrl = process.env.MIX_ADMIN_API_URL + "virtual_tours";
    this.fetchVirtualTours(`${baseUrl}?page=${page}`);
    // this.$store.dispatch("virtual_tours/fetchVirtualTours");
  },
  watch: {

            ...
            <div class="px-4 sm:px-6 lg:px-8">
                <div
                    ...
                    <div class="sm:flex-auto">
                        <h1 class="can-edu-h1">Business categories</h1>
                    </div>
                    <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                        <router-link :to="{ name: 'admin.business_categories.create' }" class="can-edu-btn-fill">
                ...
                <div
                    ...
                    <div class="relative">
                        ...
                        <form autocomplete="off" @submit.prevent>
                            <input type="text" id="table-search-business-categories"
                                ...
                                placeholder="Search for Business categories" v-model="quickSearch" />
                        </form>
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-300 overflow-auto">
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <tr class="bg-white divide-x divide-gray-200 hover:bg-gray-50"
                                v-for="business_category in business_categories" :key="business_category.id">
                                <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6 flex items-center space-x-2">
                                    <img v-if="business_category.image" class="w-10 h-10 rounded-full"
                                        :src="'/' + business_category.image.path" alt="Article Category image" />
                                ...
                                <td
                                    ...
                                    <div class="flex items-center gap-2">
                                        <router-link
                                            :to="{ name: 'admin.business_categories.edit', params: { id: business_category.id } }"
                                        ...
                                        <a :to="{ name: 'admin.business_categories.edit', params: { id: business_category.id } }"
                                            type="button" data-modal-target="editUserModal"
                                            data-modal-show="editUserModal"
                                            class="text-red-500 hover:text-red-700 cursor-pointer"
                                            @click.prevent="deleteBusinessCategory(business_category)">
                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                                stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                      ...
                      <div>
                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                          <a href="#"
                            ...
                            @click.prevent="fetchBusinessCategories(pagination.prev_page_url)">
                          ...
                          <template v-for="page in getPageNumbers()" :key="page">
                            ...
                            <a v-else href="#"
                              ...
                              }" @click.prevent="fetchBusinessCategoriesByPage(page)">
                              {{ page }}
                            </a>
                          </template>
                          ...
                          <a href="#"
                            ...
                            @click.prevent="fetchBusinessCategories(pagination.next_page_url)">
...
export default {
    ...
    methods: {
        fetchBusinessCategories(page_url) {
            if (!page_url) return;
      const url = new URL(page_url);
      const page = url.searchParams.get("page") || 1;

      this.$router.push({ query: { ...this.$route.query, page } });
            this.$store.dispatch("business_categories/fetchBusinessCategories", { url: page_url });
        },
        fetchBusinessCategoriesByPage(page) {
      const baseUrl = this.pagination.base_url || this.pagination.first_page_url.split("?")[0];
      const queryParams = `?page=${page}`;
      const fullUrl = `${baseUrl}${queryParams}`;
      this.fetchBusinessCategories(fullUrl);
    },
    // getPageNumbers() {
    //   const pages = [];
    ...
    getPageNumbers() {
      ...
      return pages;
        ...
        updateLimit(value) {
            this.$store.commit("business_categories/setLimit", value);
            this.$store.dispatch("business_categories/fetchBusinessCategories");
        },
        deleteBusinessCategory(businessCategory) {
            this.$swal
                .fire({
                    ...
                    preConfirm: (password) => {
                        return this.$store
                            .dispatch("business_categories/deleteBusinessCategory", {
        ...
        quickSearchFilter: _.debounce(function () {
            this.$store.commit("business_categories/setSearchParam", this.quickSearch);
            this.$store.dispatch("business_categories/fetchBusinessCategories");
        }, 500),
    },
    created() {
        this.$store.commit("business_categories/setSearchParam", "");
        this.$store.commit("business_categories/setLimit", 100);
    ...
    const baseUrl = process.env.MIX_ADMIN_API_URL + "business-categories";
    this.fetchBusinessCategories(`${baseUrl}?page=${page}`);
        // this.$store.dispatch("business_categories/fetchBusinessCategories");
    },
    watch: {

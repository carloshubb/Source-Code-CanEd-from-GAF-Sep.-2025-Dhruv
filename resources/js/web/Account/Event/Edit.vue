<template>
    <div class="md:col-span-9 col-span-12 border border-gray-500 rounded-md w-full overflow-hidden">
        <div class="mt-4 flex justify-between items-center gap-2 p-2">
            <h2 class="px-4 can-school-h2 text-primary" v-if="repost == true">Repost event</h2>
            <h2 class="px-4 can-school-h2 text-primary" v-if="repost == false">Edit event</h2>
        </div>
        <form class="px-4 md:px-6 lg:px-8" @submit.prevent="addUpdateForm()">
            ...
            <div class="grid my-5 grid-cols-1 md:grid-cols-2 gap-4 md:gap-6" v-for="language in languages"
              ...
              :init="editorConfig" :tinymce-script-src="tinymceScriptSrc" /> -->
                    <editor @selectionChange="handleSelectionChange(language, 'description')"
                        :ref="`description_${language.id}`" :id="`description_${language.id}`"
                        :placeholder="static_translations?.description_placeholder || ''"
                        :initial-value="form[`description`][`description_${language?.id}`]" :init="editorConfig"
            ...
            <div class="grid md:grid-cols-4 ite md:gap-6">
                ...
                <div class="md:col-span-4">
                    <div class="flex flex-wrap gap-2">
                        <div class="relative h-32 w-32 bg-gray-50 border" v-for="(image, key) in form?.images"
                            :key="key">
            ...
            <div class="mt-4" v-for="(contact, key) in form?.contacts" :key="key">
                ...
                <div class="grid my-5 md:grid-cols-2 md:gap-6">
                    <div class="relative z-0 w-full group">
                        <label for="contact_name" class="block text-base lg:text-lg">Contact person’s name and
                            title</label>
                ...
                v-text="validationErros.get(`contact_email`)"></p> -->
                    ...
                    <div class="relative z-0 w-full group">
                        ...
                        <input type="text" name="contact_phone" id="contact_phone"
                            class="mt-2 border w-full focus:ring-primary focus:border-primary focus:outline-none px-4 py-1.5 rounded border-gray-300 border-l-[5px] rounded-l border-l-primary"
                            placeholder="With area code" @input="setContacts(key, 'contact_phone', $event.target.value)"
                            :value="form['contacts'][key]['contact_phone']
                ...
                v-text="validationErros.get(`contact_phone`)"></p> -->
                    ...
                    <div class="relative z-0 w-full group">
                        <label :for="`contact-image-[${index}]`" class="block text-base lg:text-lg">Contact person’s
                            photo</label>
                        ...
                        <img v-if="contact.contact_photo" :src="contact.contact_photo"
            ...
            <br />
            <button type="submit" class="can-edu-btn-fill mb-2" v-if="repost == true">Repost event</button>
            <button type="submit" class="can-edu-btn-fill mb-2" v-if="repost == false">Update event</button>
        </form>
    </div>
...
export default {
    ...
    props:[
        "school_id", "slug", "logged_in_customer", "logged_in_user_type_create", "is_package_amount_paid_create", "lang","event_id", "repost"],
    data() {
        return {
    ...
    methods: {
      ...
      if (!this.showPopUpModal) {
        window.location.href = "/" + this.lang + "/" + this.slug + "/our-profile/our-events";
            ...
            if (this.event_id) {
                ...
                this.$store
                    .dispatch("events/fetchEvents", {
                        id: id,
                        url: `${process.env.MIX_WEB_API_URL}events/${id}?withEventDetail=1`,
                    })
                    .then((res) => {
                        console.log('event web res',res);
                        let keys = [
                            "start_date",
                            "end_date",
                            "event_website",
                        ...
                        if (res.data.data.event_images && this.repost == false) {
                            this.$store.commit("events/emptyImages");
    ...
    created() {
        ...
        this.fetchEvents();
        // this.fetchEvents();
        this.$store.commit("events/setLimit", 20000);
        ...
        this.$store
            ...
            .then((res) => {
                ...
                //     this.isDataLoaded = 1;
                //   }
                console.log('this.fetchEvents' ,this.form);
            });
        this.fetchEvents();

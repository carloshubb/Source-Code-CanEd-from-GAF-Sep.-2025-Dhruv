<template>
  ...
  <div class="px-4 sm:px-6 lg:px-8">
      <div class="flex flex-col sm:flex-col md:flex-row lg:flex-row items-center justify-between py-4 gap-2 bg-white dark:bg-gray-800">
          <div class="sm:flex-auto">
              <h2 class="can-school-h2 text-primary">Blogs</h2>
          </div>
              ...
              <tbody class="divide-y divide-gray-200 bg-white">
                  ...
                  >
                      ...
                      <!-- <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6">
                          <div class="font-medium text-gray-900">
                                        {{
                                                blog?.blog_detail?.[0]?.category_name || ''
                                            }}
                                        </div>
                      </td> -->

                      <td class="relative py-4 pl-4 pr-3 text-base sm:pl-6">
                          <div class="font-medium text-gray-900 line-clamp-2" v-html="blog?.blog_detail?.[0]?.short_description || ''"></div>
                      </td>
                      
                      <td
                          class="relative py-3.5 px-3 gap-2 whitespace-nowrap"
                      >
                          <a
                              @click.prevent="
                                  editBlog(blog)
  ...
  <BlogModal :logged_in_customer="JSON.parse(logged_in_customer) && JSON.parse(logged_in_customer)?.['id'] ? JSON.parse(logged_in_customer)?.['id'] : null" :school_id="school_id" />
</div>
</template>
...
import BlogModal from "./Modals/BlogModal.vue";
...
export default {
  ...
  methods: {
      toggleModal() {
          this.$store.commit("blogs/setShowModal", 1);
      },
      fetchBlogs(page_url, type) {
      ...
      updateLimit(value) {
          this.$store.commit("blogs/setLimit", value);
      },
      editBlog(blog) {
          this.$store.commit(
              "blogs/setBlog",
...
school_id: this.school_id,
  ...
  created() {
      ...
      this.$store.commit("blogs/setLimit", 10);
      this.$store.commit("blogs/setSortBy", "id");
      this.$store.commit("blogs/setSortType", "desc");
      this.$store.dispatch("blogs/fetchBlogs", {
              school_id: this.school_id,
          });
  },

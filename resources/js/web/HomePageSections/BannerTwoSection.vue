<template>
    <div class="mt-14" v-if="home_settings?.banner2?.banner_type == 'banner_1'">
        <div class="flex rounded-md md:rounded-lg overflow-hidden">
            <div :style="'background-image:url(' +
                home_settings?.banner2?.image1?.large_image +
                ')'
                "
                ...
                <div>
                    <p class="text-lg md:text-xl lg:text-2xl font-FuturaMdCnBT text-white leading-5">
                        {{ home_settings?.banner2?.banner_detail[0]?.title }}
                    </p>
                    <p class="text-white md:mb-4 mt-1 md:mt-2 line-clamp-2 text-xs md:text-base lg:text-lg" v-html="home_settings?.banner2?.banner_detail[0]
                        ?.banner_description
                        "></p>
                    <button
                        ...
                        <a :href="home_settings?.banner2?.banner_button_link" target="_blank">
                            {{
                                home_settings?.banner2?.banner_detail[0]
                                    ?.banner_button_text
                            }}
                        </a>
                    </button>
            ...
            <div class="w-1/2">
                <img class="object-cover w-full  h-28 md:h-48 lg:h-60 md:rounded-r-lg"
                    :src="home_settings?.banner2?.image2?.large_image" alt="" />
            </div>
        </div>
    </div>
    <a :href="formatUrl(home_settings?.banner2?.banner_button_link)" target="_blank">
        <div :style="'background-image:url(' + home_settings?.banner2?.image1?.large_image + ')'"
    ...
    <div class="mt-14">
        <div class="md:h-48 lg:h-60 rounded-lg flex overflow-hidden my-10"
            v-if="home_settings?.banner2?.banner_type == 'banner_3'">
            <div :style="'background-image:url(' +
                home_settings?.banner2?.image1?.large_image +
                ')'
                ...
                <div class="px-4 md:px-8 lg:px-16 space-y-1 md:space-y-3 py-1">
                    ...
                    <button
                        class="bg-white px-4 md:px-6 text-sm md:text-base py-0.5 md:py-1 lg:py-1.5 text-primary font-FuturaMdCnBT rounded mt-1 md:mt-3">
                        <a :href="home_settings?.banner2?.banner_button_link">
                            {{
                                home_settings?.banner2?.banner_detail[0]
                                    ?.banner_button_text
                            }}
            ...
            <div class="relative w-1/2">
                <img :src="home_settings?.banner2?.image2?.large_image"

...
const languages = {
    ...
    mutations: {
        ...
        clearValidationErrors(state, fields) {
            fields.forEach(field => {
              state.validationErros.clear(field);
            });
          }
    },
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}languages/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}languages`;
            commit("setLoading");
        ...
        fetchLanguages({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}languages?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
        ...
        fetchLanguage({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}languages/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
        ...
        deleteLanguage({ commit }, payload) {
            ...
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}languages/${payload.id}`;
            return axios
            .delete(url, {

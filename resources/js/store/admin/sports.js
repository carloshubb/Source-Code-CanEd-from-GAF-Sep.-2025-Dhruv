    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}sports/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}sports`;
            commit("setLoading");
        ...
        fetchSports({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}sports?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
                    ...
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            commit("setSports", res.data.data);
                            resolve(res);
                        }
                    })
                    .catch((error) => {
        ...
        fetchSport({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}sports/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
        ...
        deleteSport({ commit }, payload) {
            commit("setLoading");
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}sports/${payload.id}`;
            return axios
            .delete(url, {

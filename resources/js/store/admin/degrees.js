...
const degrees = {
    ...
    mutations: {
        ...
        setForm(state, payload) {
            ...
            state.form.degree_image = payload.degree_image
    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}degrees/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}degrees`;
            commit("setLoading");
        ...
        fetchDegrees({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}degrees?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            commit("setDegrees", res.data.data);
                            resolve(res);
                        }
                    })
                    .catch((error) => {
        ...
        fetchDegree({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}degrees/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
        ...
        deleteDegree({ commit }, payload) {
            ...
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}degrees/${payload.id}`;
            return axios
            .delete(url, {

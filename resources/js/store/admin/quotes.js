    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}quotes/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}quotes`;
            commit("setLoading");
        ...
        fetchQuotes({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}quotes?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
                    ...
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            commit("setQuotes", res.data.data);
                            resolve(res);
                        }
                    })
                    .catch((error) => {
        ...
        fetchQuote({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}quotes/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
        ...
        deleteQuote({ commit }, payload) {
            ...
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}quotes/${payload.id}`;
            return axios
            .delete(url, {

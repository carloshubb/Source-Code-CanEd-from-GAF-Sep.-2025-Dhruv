    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}careers/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}careers`;
            commit("setLoading");
        ...
        fetchCareers({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}careers?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
                    ...
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            commit("setCareers", res.data.data);
                            resolve(res);
                        }
                    })
                    .catch((error) => {
        ...
        fetchCareer({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}careers/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
        ...
        deleteCareers({ commit }, payload) {
            commit("setLoading");
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}careers/${payload.id}`;
            return axios
            .delete(url, {

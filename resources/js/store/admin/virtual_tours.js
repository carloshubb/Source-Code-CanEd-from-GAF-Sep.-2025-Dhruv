    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}virtual_tours/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}virtual_tours`;
            commit("setLoading");
        ...
        fetchVirtualTours({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}virtual_tours?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
        ...
        fetchVirtualTour({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}virtual_tours/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
        ...
        deleteVirtualTour({ commit }, payload) {
            ...
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}virtual_tours/${payload.id}`;
            return axios
            .delete(url, {
        ...
        deactiveVirtualTour({ commit }, payload) {
            ...
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}virtual_tours/deactive-school`;
            return new Promise(function (resolve, reject) {
                axios
                    .post(url, payload)
                    .then((res) => {

...
const registration_packages = {
    ...
    mutations: {
        ...
        addNewFeature(state, payload) {
            ...
            state.form.features[`features_${payload.languageId}`].push(
                payload.value
            );
        },
        removeFeature(state, payload) {
            state.form.features[`features_${payload.languageId}`].splice(
                payload.index,
                1
            );
        },
        ...
        setRegistrationPackages(state, payload) {
            state.registration_packages = payload;
        },
    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}registration-packages/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}registration-packages`;
            commit("setLoading");
        ...
        fetchRegistrationPackages({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}registration-packages?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
        ...
        fetchRegistrationPackage({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}registration-packages/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
        ...
        deleteRegistrationPackage({ commit }, payload) {
            commit("setLoading");
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}registration-packages/${payload.id}`;
            return axios
            .delete(url, {
...
export default registration_packages;

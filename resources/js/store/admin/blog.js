    ...
    actions: {
        ...
        fetchBlogs({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    :  `${process.env.MIX_WEB_API_URL}blogs?withBlogDetail=1&is_admin=1&school_id=${payload?.school_id}`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
        ...
        fetchBlog({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_WEB_API_URL}blogs/${payload.id}?withBlogDetail=1&is_admin=1&`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios

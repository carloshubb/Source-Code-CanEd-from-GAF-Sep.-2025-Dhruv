...
const messaging_apps = {
    ...
    mutations: {
        ...
        setMessagingApps(state, payload) {
            state.messaging_apps = payload;
        },
        ...
        setForm(state, payload) {
            ...
            state.form.messaging_app_image = payload.messaging_app_image
    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}messaging_apps/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}messaging_apps`;
            commit("setLoading");
        ...
        fetchMessagingApps({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}messaging_apps?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            commit("setMessagingApps", res.data.data);
                            resolve(res);
                        }
                    })
                    .catch((error) => {
        ...
        fetchMessagingApp({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}messaging_apps/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
        ...
        deleteMessagingApp({ commit }, payload) {
            commit("setLoading");
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}messaging_apps/${payload.id}`;
            return axios
            .delete(url, {

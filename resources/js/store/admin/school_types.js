...
const school_types = {
    ...
    mutations: {
        ...
        setSchoolTypes(state, payload) {
            state.school_types = payload;
        },
    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}school_types/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}school_types`;
            commit("setLoading");
        ...
        fetchSchoolTypes({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}school_types?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            res.data.data.sort((a, b) => {
                                const nameA = a.school_type_detail
                                    .map(detail => detail.name.toUpperCase())
                                    .sort()[0] || '';
                                const nameB = b.school_type_detail
                                    .map(detail => detail.name.toUpperCase())
                                    .sort()[0] || '';
                            ...
                            commit("setSchoolTypes", res.data.data);
                            resolve(res);
                        }
                    })
                    .catch((error) => {
        ...
        fetchSchoolType({ commit, state }, payload) {
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}school_types/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios.get(url)
        ...
        deleteSchoolTypes({ commit }, payload) {
            commit("setLoading");
            let url = payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}school_types/${payload.id}`;
            return axios
            .delete(url, {

...
const countries = {
    ...
    mutations: {
        updateCountryName(state, { name, id }) {
            console.log("name", name);
            if (!state.form.country_name) {
                state.form.country_name = {};
            }
            state.form.country_name[`country_${id}`] = name;
        },
    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_ADMIN_API_URL}countries/${state.form.id}`
                : `${process.env.MIX_ADMIN_API_URL}countries`;
            commit("setLoading");
        ...
        fetchCountries({ commit, state }, payload) {
            console.log("here");
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}countries?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
        ...
        fetchCountry({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}countries/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
        ...
        deleteCountry({ commit }, payload) {
            ...
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_ADMIN_API_URL}countries/${payload.id}`;
            return axios
                .delete(url, {

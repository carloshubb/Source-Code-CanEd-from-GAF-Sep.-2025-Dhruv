...
const programs = {
    ...
    mutations: {
        ...
        setForm(state, payload) {
            ...
            state.form.program_image = payload.program_image;
        },
    ...
    actions: {
        addUpdateForm({ commit, state }) {
            ...
            let url = state.isFormEdit
                ? `${process.env.MIX_WEB_API_URL}programs/${state.form.id}`
                : `${process.env.MIX_WEB_API_URL}programs`;
            commit("setLoading");
        ...
        fetchPrograms({ commit, state }, payload) {
            console.log(state.param);
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_WEB_API_URL}programs?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
                    ...
                    .then((res) => {
                        if (res.data.status == "Success") {
                            ...
                            const nameA = a.program_detail
                                .map(detail => detail.name.toUpperCase())
                                .sort()[0] || '';
                            const nameB = b.program_detail
                                .map(detail => detail.name.toUpperCase())
                                .sort()[0] || '';
        ...
        fetchProgram({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_WEB_API_URL}programs/${payload.id}?q=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
        ...
        deleteProgram({ commit }, payload) {
            ...
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_WEB_API_URL}programs/${payload.id}`;
            return new Promise(function (resolve, reject) {
                axios
                    .delete(url)
                    .then((res) => {

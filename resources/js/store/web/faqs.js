...
const faqs = {
    ...
    mutations: {
        ...
        setFaq(state, payload) {
            const { id, order, type,customer_id } = payload;
            state.form["id"] = id;
    ...
    actions: {
        ...
        fetchFaqs({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_WEB_API_URL}faqs?withFaqDetail=1&type=${payload?.type}`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios
        ...
        fetchFaq({ commit, state }, payload) {
            let url =
                payload && payload.url
                    ? payload.url
                    : `${process.env.MIX_WEB_API_URL}faqs/${payload.id}?withFaqDetail=1`;
            url = `${url}&${state.param}`;
            commit("setLoading");
            return new Promise(function (resolve, reject) {
                axios

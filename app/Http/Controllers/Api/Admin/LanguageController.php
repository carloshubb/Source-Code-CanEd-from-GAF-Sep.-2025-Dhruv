...
class LanguageController extends Controller
{
    ...
    public function store(LanguageRequest $request)
    {
        ...
        if (isset($request->flag_icon)) {
            $media = json_decode($request->flag_icon, true);

            if (is_array($media) && isset($media['media']) && !empty($media['media'])) {
                $files = $this->moveFile($media['media'], 'media/images', 'flag_icon');
            } else {
                return response()->json(['error' => 'Invalid media data'], 400);
            }
        } else {
            return response()->json(['error' => 'No flag icon image provided'], 400);
        }
        ...
        if ($language) {
            ...
            return $this->apiSuccessResponse(new LanguageResource($language), 'Your changes have been done successfully');
        }
    ...
    public function update(Request $request, Language $language)
    {
        ...
        if ($request->has('flag_icon') && !empty($request->flag_icon)) {
            $media = is_array($request->flag_icon)
                ? $request->flag_icon
                : json_decode($request->flag_icon, true);
    ...
    public function destroy(Request $request, Language $language)
    {
        ...
        if ($language->delete()) {
            return $this->apiSuccessResponse(new LanguageResource($language), 'The selected item has been deleted successfully');
        }
        return $this->errorResponse();
    }
    ...
    public function getCustomerLanguages(Request $request){

        $getLanguageIds = CustomerLanguage::whereCustomerId(Auth::guard('customers')->user()->id)->pluck('language_code')->toArray();
        $languages = Language::whereIn('abbreviation', $getLanguageIds)->get();

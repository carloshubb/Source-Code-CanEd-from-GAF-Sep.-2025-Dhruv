...
class StaticTranslationController extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($_GET['findByType']) && $_GET['findByType'] != '') {
            ...
            if (isset($_GET['createStaticTranslationDetail']) && $_GET['createStaticTranslationDetail'] == '1') {
                ...
                $staticTrans = StaticTranslation::where('type', $type)->with([
                    'staticTranslationDetail' => function ($q) use ($lang) {
    ...
    public function store(Request $request)
    {
        ...
        if ($defaultLang) {
            $staticTranslation = StaticTranslation::whereType($type)->with([
                'staticTranslationDetail' => function ($q) use ($defaultLang) {
        ...
        $staticTranslation = StaticTranslation::whereType($type)->first();
        foreach ($languages as $key => $language) {
            foreach ($staticTranslationDetail as $key => $translationDetail) {
                $detail = StaticTranslationDetail::whereLanguageId($language->id)->where('static_translation_id', $staticTranslation->id)->where('key', $translationDetail->key)->exists();
                if ($detail) {
                    StaticTranslationDetail::whereLanguageId($language->id)->where('static_translation_id', $staticTranslation->id)->where('key', $translationDetail->key)->update([
                    ...
                    StaticTranslationDetail::create([
                        'static_translation_id' => $staticTranslation->id,
                        'language_id' => $language->id,
